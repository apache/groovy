...
final class AsyncVirtualThreadTest {
    ...
    void testCheckedExceptionConsistencyAcrossLambdas() {
        assertScript '''
            ...
            def asyncFn = async { x -> throw new IOException("lambda error: ${x}") }
            try {
                await asyncFn("test")
                assert false : "Should have thrown"
            } catch (IOException e) {
    ...
    @Test
    void testAsyncClosureUsesCustomExecutor() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.Executors
            ...
            try {
                ...
                def threadName = await task()
                assert threadName.startsWith("exec-async-test")
            } finally {
                Awaitable.setExecutor(null)
                pool.shutdown()
            }
    ...
    @Test
    void testAsyncClosureSideEffectUsesCustomExecutor() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.Executors
    ...
    void testToAsyncStreamConversion() {
        assertScript '''
            ...
            try {
                def results = []
                def stream = AsyncStream.from([10, 20, 30])
                for await (item in stream) {
                    results << item
                }
    ...
    void testAwaitObjectDispatchesToCorrectOverload() {
        assertScript '''
            ...
            Object c = CompletableFuture.completedFuture("stage") as CompletionStage
    ...
    // ===== Executor and configuration tests =====
