...
class AsyncTransformTest {
    ...
    void testAwaitableAwaitFutureInterface() {
        assertScript '''
            ...
            async doWork() {
                def pool = Executors.newSingleThreadExecutor()
                try {
                    Future<String> future = pool.submit({ "from-executor" } as Callable)
                    def result = await future
    ...
    void testAwaitableThen() {
        assertScript '''
            ...
            def p = Awaitable.from(CompletableFuture.supplyAsync { 10 })
            def p2 = p.then { it * 3 }
            assert p2.get() == 30
        '''
    }
    ...
    void testAwaitableThenCompose() {
        assertScript '''
            ...
            def p = Awaitable.from(CompletableFuture.supplyAsync { 5 })
            def p2 = p.thenCompose { val ->
                Awaitable.from(CompletableFuture.supplyAsync { val * 4 })
            }
            assert p2.get() == 20
        '''
    }
    ...
    void testAwaitableExceptionally() {
        assertScript '''
            ...
            def p = Awaitable.from(CompletableFuture.supplyAsync { throw new RuntimeException("fail") })
    ...
    @Test
    void testAsyncStreamEmptyFactoryReturnsEmptyStream() {
        assertScript '''
            import groovy.concurrent.AsyncStream

            AsyncStream stream = AsyncStream.empty()
    ...
    @Test
    void testAwaitUnwrapsNestedExceptions() {
        assertScript '''
            ...
            async unwrap() {
                ...
                future.completeExceptionally(
                    new CompletionException(new CompletionException(new ExecutionException(root)))
                )
                await future
            }
        ...
        assertScript '''
            ...
            def asyncClosure = async {
                ...
                [
                    ...
                    virtual: thread.metaClass.respondsTo(thread, 'isVirtual') ? thread.isVirtual() : false
                ]
            }
            def threadInfo = await asyncClosure()
            assert threadInfo.name != null

            if (Awaitable.isVirtualThreadsAvailable()) {
                assert threadInfo.virtual
            } else {
                assert threadInfo.daemon
                assert threadInfo.name.startsWith('groovy-async-') : \
                    "Expected 'groovy-async-*' but got: ${threadInfo.name}"
            }
        '''
    }
