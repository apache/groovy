...
class AsyncApiTest {
    ...
    void testAwaitAllFailsFastWithoutWaitingForSlowSiblings() {
        ...
        def failure = CompletableFuture.<String>failedFuture(new IOException('awaitAll-fail-fast'))
    ...
    void testAwaitAllEmptyEdgeCases() {
        assert AsyncSupport.awaitAll() == []
        assert AsyncSupport.awaitAll(null) == []
    }
    ...
    void testAwaitAllSettled() {
        ...
        def results = AsyncSupport.awaitAllSettled(a, b)
        assert results.size() == 2
