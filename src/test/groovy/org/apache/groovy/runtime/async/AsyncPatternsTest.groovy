...
class AsyncPatternsTest {
    ...
    // =========================================================================
    // Parallel execution: Awaitable.all / Awaitable.any
    // =========================================================================
    ...
    void testAwaitVsGetExceptionBehavior() {
        assertScript '''
            ...
            try {
                ...
                awaitCaughtType = e.class
            }
            assert awaitCaughtType == IllegalStateException
            ...
            try {
                ...
                getCaughtType = e.class
            }
            assert getCaughtType == ExecutionException
        '''
    }
    ...
    void testThenComposeFlattensNestedAwaitable() {
        assertScript '''
            ...
            async fetchId() { 42 }
            async fetchName(int id) { "user-${id}" }
    ...
    @Test
    void testAwaitableCompletionFlags() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture

            def source = new CompletableFuture<String>()
            def awaitable = Awaitable.from(source)
            ...
            def failedSource = new CompletableFuture<String>()
            def failed = Awaitable.from(failedSource)
            failedSource.completeExceptionally(new RuntimeException("err"))
    ...
    @Test
    void testAwaitableFromCompletableFutureFactory() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture

            Awaitable<String> awaitable = Awaitable.from(CompletableFuture.completedFuture("hello"))
...
assert completed.get() == 1000
    ...
    void testPromiseCancelledGet() {
        assertScript '''
            ...
            def task = Awaitable.go { Thread.sleep(10000); 'done' }
            assert task.cancel()
            try {
                task.get()
    ...
    @Test
    void testPromiseCancelAfterDone() {
        assertScript '''
            import groovy.concurrent.Awaitable

            def task = Awaitable.of('already done')
            assert !task.cancel()
            assert task.isDone()
            assert !task.isCancelled()
            assert task.get() == 'already done'
        '''
    }
    ...
    void testPromiseToString() {
        assertScript '''
            ...
            def done = Awaitable.of(42)
            assert done.toString().contains('completed')
            assert done.isDone()
            assert done.get() == 42

            def failed = Awaitable.failed(new RuntimeException('err'))
            assert failed.toString().contains('failed')
            assert failed.isDone()

            def pending = Awaitable.go { Thread.sleep(5000); 'x' }
    ...
    @Test
    void testPromiseThenException() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async thenExTest() {
                def p = Awaitable.of(10)
                def chained = p.then { throw new IllegalStateException("then error") }
                try {
                    await chained
                    assert false
                } catch (IllegalStateException e) {
                    assert e.message == 'then error'
                }
            }
    ...
    @Test
    void testPromiseThenCompose() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async composeTest() {
                def p = Awaitable.of(5)
                def result = await p.thenCompose { v -> Awaitable.of(v * 2) }
                assert result == 10
            }
    ...
    @Test
    void testPromiseThenComposeFailure() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async composeFailTest() {
                def p = Awaitable.of(5)
                def chained = p.thenCompose { throw new RuntimeException("compose fail") }
                try {
                    await chained
                    assert false
                } catch (RuntimeException e) {
                    assert e.message == 'compose fail'
                }
            }
    ...
    @Test
    void testPromiseExceptionallyHandler() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async exceptionallyTest() {
                def p = Awaitable.failed(new RuntimeException("original"))
                def recovered = p.exceptionally { err -> "recovered from: ${err.message}" }
                assert await(recovered) == 'recovered from: original'
            }

            exceptionallyTest().get()
        '''
    }

    @Test
    void testPromiseHandleBothPaths() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async handleTest() {
                def success = Awaitable.of(42)
                def r1 = await success.handle { val, err -> err == null ? val * 2 : -1 }
                assert r1 == 84

                def failure = Awaitable.failed(new RuntimeException("oops"))
                def r2 = await failure.handle { val, err -> err != null ? err.message : 'ok' }
                assert r2 == 'oops'
            }
    ...
    @Test
    void testAsyncStreamYieldAfterEarlyClose() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async yieldCloseTest() {
                def items = []
                for await (item in yieldGen()) {
                    items << item
                    if (items.size() >= 2) break  // close stream after 2 items
                }
                assert items == [1, 2]
            }
    ...
    @Test
    void testAsyncStreamGeneratorMoveNextAfterClose() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async moveNextCloseRace() {
                def stream = slowGen()
                assert await stream.moveNext()
                assert stream.current == 'a'

                // Close early while producer might be waiting
                stream.close()
            ...
            moveNextCloseRace().get()
        '''
    }

    @Test
    void testAbstractAsyncStreamErrorSignalChecked() {
        assertScript '''
            import groovy.concurrent.Awaitable
                    ...
                    subscriber.onSubscribe(new Flow.Subscription() {
                        ...
                        void request(long n) {
                            if (!sent) {
                                ...
                                subscriber.onError(new IOException("stream IO error"))
                            }
                        }
                        void cancel() {}
                    })
            ...
            async errorStreamTest() {
                ...
                try {
                    ...
                    assert e.message == 'stream IO error'
                }
                assert items == ['first']
            }
