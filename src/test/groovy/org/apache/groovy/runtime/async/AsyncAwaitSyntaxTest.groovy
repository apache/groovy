...
class AsyncAwaitSyntaxTest {
    ...
    @Test
    void testForAwaitBreakReleasesBlockedGeneratorThread() {
        assertScript '''
            import java.util.concurrent.atomic.AtomicBoolean

            class Svc {
                // The generator yields infinitely; the consumer breaks after 2.
                // Without close() interrupting the blocked producer, the
                // generator thread would hang in yield() forever.
                async firstTwo() {
                    def results = []
                    def gen = async {
                        int i = 0
            ...
            assert new Svc().firstTwo().get() == [0, 1]
        '''
    }
    ...
    void testForAwaitWithTraditionalForSyntaxFails() {
        ...
        '''
        assert err.message.contains('for await requires enhanced for syntax')
    }
    ...
    void testAsyncOnClassDeclarationFails() {
        def err = shouldFail CompilationFailedException, '''
            async class Svc {}
        '''
        // Parser rejects 'async' on top-level class declarations (ASYNC not in classOrInterfaceModifier)
    ...
    void testAsyncOnInnerClassDeclarationFails() {
        ...
        '''
        assert err.message.contains('modifier `async` is not allowed for type declarations')
    }
    ...
    void testAsyncOnFieldDeclarationFails() {
        ...
        '''
        assert err.message.contains('modifier `async` is not allowed for variable declarations')
    }
    ...
    @Test
    void testAsyncOnMethodParameterFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                def work(async int n) { n }
            }
        '''
    ...
    @Test
    void testDuplicateMethodModifierFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                public public def run() { 1 }
            }
        '''
        assert err.message.contains('Cannot repeat modifier')
    }

    @Test
    void testConflictingVisibilityModifiersFail() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                public private def run() { 1 }
            }
        '''
        assert err.message.contains('access scope has already been defined')
    }
    ...
    void testAsyncOnConstructorFails() {
        ...
        '''
        assert err.message.contains('incorrect modifier') || err.message.contains('Async')
    }

    @Test
    void testAsyncKeywordOnConstructorFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                async Svc() {}
            }
    ...
    @Test
    void testVarMethodDeclarationFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                var run() { 1 }
            }
        '''
        assert err.message.contains('var cannot be used for method declarations')
    }

    @Test
    void testMethodWithoutModifierOrReturnTypeFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                run() { 1 }
            }
        '''
        assert err.message.contains('Modifiers or return type is required')
                || err.message.contains('Invalid method declaration')
    }

    @Test
    void testMethodWithoutBodyFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                public run()
            }
        '''
        assert err.message.contains('Method body is required')
                || err.message.contains('without a body')
    }

    @Test
    void testAbstractMethodWithBodyFails() {
        def err = shouldFail CompilationFailedException, '''
            abstract class Svc {
                abstract def run() { 1 }
            }
        '''
        assert err.message.contains('Abstract method should not have method body')
                || err.message.contains('with a body')
    }

    @Test
    void testTraitAbstractMethodWithBodyFails() {
        def err = shouldFail CompilationFailedException, '''
            trait T {
                abstract def run() { 1 }
            }
    ...
    @Test
    void testAnnotationElementWithBodyFails() {
        def err = shouldFail CompilationFailedException, '''
            @interface Flag {
                int value() { 1 }
            }
        '''
        assert err.message.contains('Annotation type element should not have body')
    }

    @Test
    void testRecordWithoutHeaderFails() {
        def err = shouldFail CompilationFailedException, '''
            record Rec { }
        '''
        assert err.message.contains('header declaration of record is expected')
    }

    @Test
    void testSealedRecordDeclarationFails() {
        def err = shouldFail CompilationFailedException, '''
            sealed record Rec(int x) { }
        '''
        assert err.message.contains('`sealed` is not allowed for record declaration')
    }

    @Test
    void testNonSealedRecordDeclarationFails() {
        def err = shouldFail CompilationFailedException, '''
            non-sealed record Rec(int x) { }
        '''
        assert err.message.contains('`non-sealed` is not allowed for record declaration')
    }

    @Test
    void testDeferOutsideAsyncMethodFails() {
        def err = shouldFail CompilationFailedException, '''
            class Svc {
                def normalMethod() {
                    defer { }
        ...
        '''
        assert err.message.contains('`defer` can only be used inside an async method or async closure')
    }

    @Test
    void testDeferOutsideAsyncClosureFails() {
        def err = shouldFail CompilationFailedException, '''
            def fn = { ->
                defer { }
                "done"
    ...
    @Test
    void testDeferAtScriptTopLevelFails() {
        def err = shouldFail CompilationFailedException, '''
            defer { }
        '''
        assert err.message.contains('`defer` can only be used inside an async method or async closure')
    ...
    void testYieldReturnOutsideAsyncMethodFailsCompileTime() {
        ...
        '''
        assert err.message.contains('`yield return` can only be used inside an async method or async closure')
    }
    ...
    void testDeferInsideAsyncMethodSucceeds() {
        assertScript '''
            ...
            assert await(new Svc().doWork()) == 42
        '''
    }

    @Test
    void testDeferInsideAsyncClosureSucceeds() {
        assertScript '''
            def task = async {
                defer { }
                "done"
    ...
    // ===== Async syntax integration tests =====
    ...
    @Test
    void testForAwaitWithTypedVariableDeclaration() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async typedForAwait() {
                def items = []
                for await (Integer item in typedGen()) {
                    items << item
                }
                assert items == [1, 2]
            }
    ...
    void testCompileStaticDeferWithAsyncCleanup() {
        assertScript '''
            ...
            assert await(new QuestCleanup().run()) == ['body', 'close:inner', 'close:outer']
        '''
    }
            ...
            class QuestDashboard {
                ...
                async Map<String, Object> load(int id) {
                    return AsyncScope.withScope { scope ->
                        Awaitable<String> nameTask = scope.async { await fetchName(id) }
                        Awaitable<Integer> levelTask = scope.async { await fetchLevel(id) }
                        [name: await(nameTask), level: await(levelTask)]
                    }
                }
            }

            assert new QuestDashboard().load(2).get() == [name: 'hero-2', level: 42]
        '''
    }
    ...
    void testMultiArgAwaitTwoArgs() {
        assertScript '''
            ...
            async fetchPair() {
                def (a, b) = await(Awaitable.of('hello'), Awaitable.of('world'))
                return "$a $b"
            }
    ...
    void testMultiArgAwaitPreservesOrder() {
        assertScript '''
            ...
            async ordered() {
                // slower task first, faster task second — results must match argument order
    ...
    void testMultiArgAwaitWithDestructuring() {
        assertScript '''
            ...
            async destruct() {
                def (x, y, z) = await(Awaitable.of(1), Awaitable.of(2), Awaitable.of(3))
                return x + y + z
            }
    ...
    // ---- Unparenthesized multi-argument await --------------------------------

    @Test
    void testUnparenthesizedMultiArgAwaitBasic() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async fetchAll() {
                ...
                def p2 = Awaitable.of('beta')
                def p3 = Awaitable.of('gamma')
                // Unparenthesized multi-arg await: await a, b, c
                def results = await p1, p2, p3
                return results
            }

            assert await(fetchAll()) == ['alpha', 'beta', 'gamma']
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitTwoArgs() {
        assertScript '''
            import groovy.concurrent.Awaitable
    ...
    @Test
    void testUnparenthesizedMultiArgAwaitWithReturn() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async compute() {
                ...
                def c = Awaitable.of(30)
                return await a, b, c
            }

            assert await(compute()) == [10, 20, 30]
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitEquivalentToParenthesized() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            assert await(viaParen()) == await(viaNoParen())
            assert await(viaNoParen()) == [1, 2, 3]
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitWithDestructuring() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async destruct() {
                def (x, y, z) = await Awaitable.of(10), Awaitable.of(20), Awaitable.of(30)
                return x + y + z
            }

            assert await(destruct()) == 60
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitPreservesOrder() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture

            async ordered() {
                def p1 = CompletableFuture.supplyAsync { Thread.sleep(30); 'first' }
                def p2 = CompletableFuture.supplyAsync { 'second' }
                def p3 = CompletableFuture.supplyAsync { Thread.sleep(10); 'third' }
                // Results must follow declaration order, not completion order
                return await p1, p2, p3
            }

            assert await(ordered()) == ['first', 'second', 'third']
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitSingleArg() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async single() {
                // Single-arg — still works with no comma
                return await Awaitable.of(42)
            }
    ...
    @Test
    void testUnparenthesizedMultiArgAwaitMixedTypes() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture

            async mixed() {
                def aw = Awaitable.of('awaitable')
                def cf = CompletableFuture.completedFuture('future')
                return await aw, cf
            }

            assert await(mixed()) == ['awaitable', 'future']
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitInAsyncClosure() {
        assertScript '''
            import groovy.concurrent.Awaitable

            def fn = async {
                def a = Awaitable.of(100)
                def b = Awaitable.of(200)
                def c = Awaitable.of(300)
                return await a, b, c
            }

            assert await(fn()) == [100, 200, 300]
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitWithMethodCalls() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            class Svc {
                ...
                async fetchBoth() {
                    return await fetchA(), fetchB()
                }
            }

            assert await(new Svc().fetchBoth()) == ['alpha', 'beta']
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitCompileStatic() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import groovy.transform.CompileStatic
            ...
            assert await(new StaticSvc().compute()) == [1, 2, 3]
        '''
    }

    @Test
    void testParenthesizedAwaitInListLiteralDisambiguation() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async buildList() {
                ...
                def b = Awaitable.of(2)
                def c = Awaitable.of(3)
                // Parenthesized form disambiguates: each await is independent
                return [await(a), await(b), await(c)]
            }

            assert await(buildList()) == [1, 2, 3]
        '''
    }

    @Test
    void testParenthesizedAwaitInMapLiteralDisambiguation() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async buildMap() {
                ...
                // Parenthesized form disambiguates in map literals
                return [name: await(nameTask), age: await(ageTask)]
            }

            assert await(buildMap()) == [name: 'Alice', age: 30]
        '''
    }

    // ---- Comprehensive await syntax tests ----

    @Test
    void testUnparenthesizedMultiArgAwaitInAssignment() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async compute() {
                ...
                def b = Awaitable.of(20)
                def c = Awaitable.of(30)
                // Unparenthesized multi-arg in simple assignment
    ...
    @Test
    void testUnparenthesizedMultiArgAwaitWithExpressions() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                // Expressions as arguments to unparenthesized await
                def results = await Awaitable.of(1 + 1), Awaitable.of(2 * 3), Awaitable.of(10 - 3)
                return results
            }

            assert await(compute()) == [2, 6, 7]
        '''
    }

    @Test
    void testUnparenthesizedMultiArgAwaitWithMethodCallArgs() {
        assertScript '''
            import groovy.concurrent.Awaitable

            Awaitable<Integer> makeTask(int n) { Awaitable.of(n * 10) }

            async compute() {
                // Method calls as arguments
                def results = await makeTask(1), makeTask(2), makeTask(3)
                return results
            }
    ...
    @Test
    void testMultiArgAwaitParenAndNoParenEquivalence() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async withParens() {
                def a = Awaitable.of('x')
                def b = Awaitable.of('y')
                return await(a, b)
            }
            ...
            assert await(withParens()) == await(withoutParens())
        '''
    }

    @Test
    void testMultiArgAwaitWithTwoArgs() {
        assertScript '''
            import groovy.concurrent.Awaitable
    ...
    @Test
    void testMultiArgAwaitWithFiveArgs() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                return await Awaitable.of(1), Awaitable.of(2), Awaitable.of(3), Awaitable.of(4), Awaitable.of(5)
            }

            assert await(compute()) == [1, 2, 3, 4, 5]
        '''
    }

    @Test
    void testMultiArgAwaitResultDestructuring() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                def (a, b, c) = await Awaitable.of(10), Awaitable.of(20), Awaitable.of(30)
                return a + b + c
            }

            assert await(compute()) == 60
        '''
    }

    @Test
    void testMultiArgAwaitExceptionPropagation() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                def ok = Awaitable.of(1)
                def failing = Awaitable.failed(new RuntimeException('multi-arg fail'))
                return await ok, failing
            }
            ...
            try {
                ...
                assert e.message == 'multi-arg fail'
            }
        '''
    }

    @Test
    void testSingleArgAwaitAllForms() {
        assertScript '''
            import groovy.concurrent.Awaitable
            ...
            async test() {
                ...
                // Form 1: await with space (no parens)
                def r1 = await task

                // Form 2: await with parens
                def r2 = await(Awaitable.of(42))

                return [r1, r2]
            }

            assert await(test()) == [42, 42]
        '''
    }

    @Test
    void testAwaitableFirstInAsyncContext() {
        assertScript '''
            import groovy.concurrent.Awaitable
    ...
    @Test
    void testAwaitableFirstWithFailuresInAsyncContext() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                def failing1 = Awaitable.failed(new RuntimeException('e1'))
                def failing2 = Awaitable.failed(new RuntimeException('e2'))
                def ok       = Awaitable.go { Thread.sleep(50); 'success' }
                return await Awaitable.first(failing1, failing2, ok)
            }

            assert await(compute()) == 'success'
        '''
    }

    @Test
    void testAllAsyncSingleElementInAsyncContext() {
        assertScript '''
            import groovy.concurrent.Awaitable

            async compute() {
                def task = Awaitable.of('only')
                return await Awaitable.all(task)
            }

            assert await(compute()) == ['only']
        '''
    }
}
