...
class AsyncTransformTest {
        ...
        assertScript '''
            ...
            class Service {
                async fetchAll() {
                    ...
                    return AsyncUtils.awaitAll(f1, f2, f3)
                }
            }
    ...
    void testToAsyncStreamWithNullReturnsEmpty() {
        assertScript '''
            ...
            AsyncStream stream = AsyncUtils.toAsyncStream(null)
            assert await(stream.moveNext()) == false
        '''
    }
    ...
    void testDeepUnwrapWithNestedExceptions() {
        assertScript '''
            ...
            assert AsyncUtils.deepUnwrap(l3).is(root)
            assert AsyncUtils.deepUnwrap(root).is(root)

            // Non-wrapper exception is returned as-is
            def re = new RuntimeException("direct")
            assert AsyncUtils.deepUnwrap(re).is(re)
        '''
    }
    ...
    void testCustomExecutorOverrideAndReset() {
        assertScript '''
            ...
            try {
                ...
                def threadName = AsyncUtils.await(__t())
                assert threadName == 'my-custom-async'
            } finally {
                AsyncUtils.setExecutor(original)
