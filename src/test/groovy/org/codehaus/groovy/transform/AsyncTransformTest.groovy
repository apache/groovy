 ...
 * Awaitable abstraction, for-await, and Awaitable.
 */
class AsyncTransformTest {
    ...
    @Test
    void testAsyncAnnotationYieldReturnPrimitiveType() {
        assertScript '''
    ...
    void testAwaitableAwait() {
        assertScript '''
            ...
            assert doWork().get() == "hello"
        '''
    }

    @Test
    void testAwaitableAwaitAll() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture
            ...
            async def doWork() {
                ...
                def results = await Awaitable.all(f1, f2, f3)
                assert results == [1, 2, 3]
                results
            }
            assert doWork().get() == [1, 2, 3]
        '''
    }

    @Test
    void testAwaitableAwaitAny() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.CompletableFuture
            ...
            async def doWork() {
                ...
                def result = await Awaitable.any(f1, f2, f3)
                assert result == "first"
                result
            }
            assert doWork().get() == "first"
        '''
    }

    @Test
    void testAwaitableSetCustomExecutor() {
        assertScript '''
            import groovy.concurrent.Awaitable
            import java.util.concurrent.Executors
            ...
            try {
                ...
                def asyncClosure = async { Thread.currentThread().name }
                def future = asyncClosure()
    ...
    void testAwaitableAwaitFutureInterface() {
        assertScript '''
            ...
            assert doWork().get() == "from-executor"
        '''
    }

    @Test
    void testAwaitableAwaitExceptionUnwrapping() {
        assertScript '''
            import java.util.concurrent.CompletableFuture
    ...
    void testAsyncProducerConsumerPattern() {
        assertScript '''
            ...
            class Consumer {
                async consume(Awaitable<String> itemFuture) {
    ...
    void testAsyncWithMethodParameters() {
        assertScript '''
            ...
            class Service {
                async greet(String name, int times) {
                    def parts = []
    ...
    void testAsyncWithCollectionProcessing() {
        assertScript '''
            ...
            class BatchProcessor {
                async processBatch(List<Integer> items) {
        ...
        assertScript '''
            ...
            class Service {
                async fetchAll() {
                    ...
                    return await(Awaitable.all(f1, f2, f3))
                }
            }
    ...
    void testFullAsyncAwaitPipeline() {
        assertScript '''
            ...
            class DataService {
                async fetchUser(long id) {
                    return [name: "User${id}"]
                }

                async fetchOrders(long id) {
                    return ["order1", "order2"]
                }

                async getUserSummary(long id) {
    ...
    void testToAsyncStreamWithNullReturnsEmpty() {
        assertScript '''
            ...
            AsyncStream stream = org.apache.groovy.runtime.async.AsyncSupport.toAsyncStream(null)
            assert await(stream.moveNext()) == false
        '''
    }
    ...
    void testDeepUnwrapWithNestedExceptions() {
        assertScript '''
            ...
            assert AsyncSupport.deepUnwrap(l3).is(root)
            assert AsyncSupport.deepUnwrap(root).is(root)

            // Non-wrapper exception is returned as-is
            def re = new RuntimeException("direct")
            assert AsyncSupport.deepUnwrap(re).is(re)
        '''
    }
    ...
    void testFallbackPoolUsesNamedDaemonThreads() {
        assertScript '''
            ...
            def result = asyncClosure()
            def threadName = await(result)
    ...
    void testCustomExecutorOverrideAndReset() {
        assertScript '''
            ...
            try {
                ...
                assert Awaitable.getExecutor().is(customPool)

                def __t = async { Thread.currentThread().name }
                def threadName = await(__t())
