...
final class AsyncExceptionHandlingTest {
    ...
    void testAwaitAnyWithFailure() {
        assertScript '''
            ...
            async caller() {
                try {
                    await(Awaitable.any(Awaitable.failed(new java.io.IOException("fail"))))
                    assert false
                } catch (java.io.IOException e) {
                    return e.message
                }
    ...
    // =========================================================================
    // 16. deepUnwrap exposed via AsyncSupport (package-private, test via reflection or direct)
    // =========================================================================
    ...
    void testDeepUnwrapUtility() {
        assertScript '''
            ...
            def unwrapped = AsyncSupport.deepUnwrap(l3)
    ...
    void testDeepUnwrapHandlesNullCause() {
        assertScript '''
            ...
            def result = AsyncSupport.deepUnwrap(ce)
            assert result.is(ce)
        '''
    }
