...
class Groovy3839Bug extends GroovyTestCase {
    ...
    void testGroovyASTTransformationWithNeitherTransClassNamesNorClasses() {
        try {
            ...
            assert ex.message.contains('@GroovyASTTransformationClass in groovy.bugs.vm5.G3839A3 does not specify any transform class names/classes')
        }
    }
    ...
    void testGroovyASTTransformationWithBothTransClassNamesAndClasses() {
        try {
            ...
            assert ex.message.contains('@GroovyASTTransformationClass in groovy.bugs.vm5.G3839A4 should specify transforms only by class names or by classes and not by both')
        }
    }
}
