...
final class SwitchPatternMatchingTest {
    ...
    @Test
    void testNonExhaustivePatternSwitchRejectedWhenTypeChecked() {
        // like any other switch expression, a pattern switch must be exhaustive
        def err = shouldFail '''
            import groovy.transform.TypeChecked
            @TypeChecked
            def m(Object o) {
    ...
    @Test
    void testIncompleteSealedHierarchyCoverageRejected() {
        def err = shouldFail '''
            import groovy.transform.TypeChecked
            sealed interface Shape permits Circle, Square {}
            final class Circle implements Shape { double radius = 1 }
    ...
    @Test
    void testListPatternSwitchNeedsDefaultWhenTypeChecked() {
        // a list pattern is always conditional, so it cannot make the switch exhaustive
        def err = shouldFail '''
            import groovy.transform.TypeChecked
            @TypeChecked
            def m(List l) {
