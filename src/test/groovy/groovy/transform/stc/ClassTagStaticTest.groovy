...
final class ClassTagStaticTest extends StaticTypeCheckingTestCase {
    ...
    void testPreemptionRolledBackWhenTagOverloadDoesNotApply() {
        // injection matches by arity only, so an allowlisted name can arity-match a tag overload
        // whose remaining parameters reject the supplied argument types; the retried selection
        // fails and the injection must be rolled back so the original lenient match still binds
        def result = evalCompileStatic(['grow'] as Set, '''
            import groovy.transform.stc.ClassTag
            class Holder<K,V> {
                String grow(Closure c) { 'lenient' }
                String grow(@ClassTag Class<K> keyType, String s) { 'checked' }
            }
            Holder<Number,String> h = new Holder<>()
            h.grow{ }
        ''')
