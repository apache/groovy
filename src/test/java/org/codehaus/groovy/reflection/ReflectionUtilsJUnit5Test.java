...
class ReflectionUtilsJUnit5Test {
    ...
    void testGetCallingClassWithExtraIgnoredPackages() {
        ...
        Class<?> callingClass = ReflectionUtils.getCallingClass(1, extraIgnored);
        assertNotNull(callingClass);
    }

    @Test
    void testGetCallingClassWithEmptyIgnoredPackages() {
        Class<?> callingClass = ReflectionUtils.getCallingClass(1, Collections.emptySet());
        assertNotNull(callingClass);
    }

    @Test
    void testGetDeclaredMethods() {
        List<Method> methods = ReflectionUtils.getDeclaredMethods(String.class, "substring", int.class);
    ...
    void testGetDeclaredMethodsWithTwoParams() {
        List<Method> methods = ReflectionUtils.getDeclaredMethods(String.class, "substring", int.class, int.class);
        assertNotNull(methods);
        assertFalse(methods.isEmpty());
    }
    ...
    void testGetDeclaredMethodsNonExistent() {
        List<Method> methods = ReflectionUtils.getDeclaredMethods(String.class, "nonExistentMethod");
        assertNotNull(methods);
    ...
    @Test
    void testGetMethods() {
        List<Method> methods = ReflectionUtils.getMethods(String.class, "length");
        assertNotNull(methods);
        assertFalse(methods.isEmpty());
    }
    ...
    void testGetMethodsWithParams() {
        List<Method> methods = ReflectionUtils.getMethods(String.class, "charAt", int.class);
        assertNotNull(methods);
        assertFalse(methods.isEmpty());
    }
    ...
    void testGetMethodsInherited() {
        ...
        List<Method> methods = ReflectionUtils.getMethods(String.class, "toString");
        assertNotNull(methods);
        assertFalse(methods.isEmpty());
    }
    ...
    void testCheckCanSetAccessible() {
        try {
            Method method = String.class.getDeclaredMethod("length");
            boolean result = ReflectionUtils.checkCanSetAccessible(method, ReflectionUtilsJUnit5Test.class);
