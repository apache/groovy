...
=== Guarding against runaway regular expressions

Java's regex engine has no native timeout. A pattern subject to _catastrophic backtracking_
can take practically forever on adversarial input, hanging the evaluating thread -- a Regular
Expression Denial of Service (ReDoS). When either the pattern or the input comes from an
untrusted source, consider using gapi:groovy.util.regex.RegexGuard[], which cuts off a
runaway evaluation by throwing a gapi:groovy.util.regex.RegexTimeoutException[] once a
deadline passes. It can guard a region of code or an individual piece of data.

The `guard(timeout) { ... }` form applies a per-evaluation timeout to every regex evaluation
reached through Groovy's regex operators or `CharSequence` extension methods for the dynamic
extent of a closure (on the same thread, including within called methods), and returns the
closure's result:

[source,groovy]
----
include::../test/OperatorsTest.groovy[tags=regex_guard_scope,indent=0]
----

Well-behaved evaluations are unaffected; a runaway one is cut off shortly after the deadline:

[source,groovy]
----
include::../test/OperatorsTest.groovy[tags=regex_guard_scope_timeout,indent=0]
----

Alternatively, `guard(input, timeout)` attaches a single total deadline to the data itself.
Wrap untrusted input once at a trust boundary and pass it around freely; every regex
evaluated against it, wherever that happens, shares the one budget:

[source,groovy]
----
include::../test/OperatorsTest.groovy[tags=regex_guard_input,indent=0]
----

For guarding individual expressions, `RegexGuard.matches(pattern, input, timeout)` and
`RegexGuard.matcher(pattern, input, timeout)` are guarded equivalents of the match and find
operators, and the <<core-metaprogramming.adoc#xform-SafeRegex,`@SafeRegex`>> AST transform
rewrites the operators within an annotated scope to equivalent guarded calls automatically
(via the operator-shaped `matchRegex(input, pattern, timeout)` and
`findRegex(input, pattern, timeout)` entry points, which keep the operators' left-to-right
operand evaluation order).

[NOTE]
The guard mechanism works by checking a deadline while the regex engine reads input
characters, so it requires the evaluation to go through Groovy's runtime or over guarded
input. Direct JDK calls bypass it: on a `String` receiver, `matches(String)`,
`replaceAll(String, String)`, `replaceFirst(String, String)` and `split(String)` dispatch to
`java.lang.String` itself. Their `Pattern`-argument variants (e.g.
`str.replaceAll(~/regex/, 'x')`) are Groovy extension methods and are covered.
...
As of GROOVY-12319, diamond may also be used when creating an anonymous inner class, matching Java 9 (JEP 213):

[source,groovy]
----
include::../test/OperatorsTest.groovy[tags=diamond_op_aic,indent=0]
----
...
Compound assignment operators (`+=`, `-=`, `\*=`, ...) normally desugar to `x = x.op(y)`,
which creates a new value and reassigns the variable. Since Groovy 6.0 (GEP-15), classes can
define dedicated *Assign methods to override compound-assign independently of the base operator.
When a `*Assign` method exists on the receiver type and the compiler can resolve it directly
(for example, under `@CompileStatic` or `@TypeChecked`), it is called directly: the receiver is
mutated in place, no reassignment occurs (so `final` fields and variables can use compound
assignment in that case), and for property LHS the setter is *not* invoked. In purely dynamic
code the dispatch happens at runtime; the in-place behaviour still applies but the
`final`-reassignment check cannot be relaxed because the compiler does not yet know whether
a `*Assign` method will respond at runtime.

If no `*Assign` method matches, the legacy `x = x.op(y)` desugar still applies.
...
When `*Assign` is called, the expression value of `x op= y` is the (mutated) `x`, not the
return value of `*Assign`. `*Assign` is also discoverable as an extension method or category,
and its name can be remapped via `@OperatorRename(plusAssign="addInPlace")`.
