...
Triple single quoted strings are a series of characters surounded by single quotes:
...
NOTE: Option <1> is interesting when the character is held in a variable, but <2> and <3> are more interesting when a char value must be passed as argument of a method call.
