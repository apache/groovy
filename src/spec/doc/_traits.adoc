...
* internally, the trait is represented as an interface (without default methods; public static methods are exposed as JVM-native interface statics) and several helper classes
...
NOTE: Static member support in traits is *incubating*: the behaviour described here is settled for {groovyVersion}
but may still be refined. GEP-22 gives the full specification.

A trait may declare static methods, properties and fields:

* The trait is interpreted as a _template_ for the implementing class: each implementing class gets its *own copy*
of the trait's static members. A static member declared on a trait belongs to the implementing class, not to a
single copy shared across all implementers.
* Public static methods are _declarer-bound_ by default and are promoted onto the generated trait interface as
JVM-native static methods, so both `Trait.m(...)` and `Impl.m(...)` resolve. A private static method stays internal
to the trait and is not promoted.
* Static methods may be declared in, and called from, `@CompileStatic` and `@TypeChecked` code; they are not
restricted to dynamic Groovy.
* You should typically not mix static and instance methods of the same signature. The normal trait method-selection
rules apply (including multiple inheritance conflict resolution): if the selected method is static but some
implemented trait has an instance variant, a compilation error occurs; if the instance variant is selected, the
static variant is ignored (as with static methods in Java interfaces for this case).
...
=== Overridable static defaults with `@Virtual`

By default a trait's static method is _declarer-bound_: when other trait code calls it, it always resolves to the
trait's own copy, even if an implementing class declares a static method of the same signature. That is the right
choice for an internal helper or a constant that the trait owns outright.

Sometimes, though, a trait wants to publish a static *default that an implementing class can override*, and have
the trait's own logic honour that override. This is the template-method pattern applied to a static member — the
same override-visibility that trait _instance_ methods already have. Annotate the method with
`@groovy.transform.Virtual` to opt into per-implementer override dispatch:

[source,groovy]
----
include::../test/TraitsSpecificationTest.groovy[tags=virtual_default,indent=0]
----
<1> the overridable default, marked `@Virtual`
<2> trait logic that consults the setting
<3> an unqualified call, so it dispatches through the implementing class
<4> uses the trait's default
<5> overrides the default with its own `static`
<6> the default is seen for `StrictEntity`
<7> the override is seen by the trait's own `describe()` for `LenientEntity`
<8> and externally via `Impl.m()`

Reach for `@Virtual` when the overridable thing is genuinely a property of the _type_ rather than of an instance —
a policy or configuration default that answers a question about the class as a whole (Grails uses this for hooks
such as `Validateable.defaultNullable()`), that may be consulted where no instance exists yet (class
initialisation, static factory methods, building metadata), and that external callers can read statically as
`Impl.setting()`. For overridable static _data_ rather than behaviour, use a `@Virtual` accessor
(`@Virtual static getSetting()`) rather than a static field, since a bare static-field read from trait code is not
override-visible (see <<_inheritance_of_state_gotchas,Inheritance of state gotchas>>).

A `@Virtual` static is deliberately *not* exposed on the trait interface (interface statics are declarer-bound by
JVM rule, which is incompatible with override dispatch). It therefore has no qualified `Trait.m(...)` form: read a
concrete class's value with `Impl.m(...)`, or use `T.super.m(...)` from within trait code for the trait's own
default. A qualified `Trait.m(...)` call to a `@Virtual` static is rejected at compile time:

[source,groovy]
----
include::../test/TraitsSpecificationTest.groovy[tags=virtual_qualified,indent=0]
----
<1> rejected: a `@Virtual` static has no qualified `Trait.m(...)` form
