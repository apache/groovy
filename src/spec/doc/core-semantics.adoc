...
Groovy also supports switch expressions as specified by
https://openjdk.org/jeps/361[JEP 361: Switch Expressions]
(link:https://issues.apache.org/jira/browse/GROOVY-12255[GROOVY-12255]).
A switch expression produces a value. Arms may use arrow labels
(`case L ->`), which do not fall through, or traditional colon labels
(`case L:`) with an explicit `yield`. A switch expression must complete by
yielding a value or by throwing; unlike a switch statement, an unmatched
selector throws `IllegalStateException`. Under `@TypeChecked` /
`@CompileStatic` a non-exhaustive switch expression (no `default`, and not a
complete enum) is a compile-time error.

As in Java, the arrow only decides fall-through; whether a `switch` is a
statement or an expression is decided by its position
(link:https://issues.apache.org/jira/browse/GROOVY-12399[GROOVY-12399]).
A `switch` whose value is not used is a statement, whatever its arm shape,
and simply does nothing when unmatched. That includes a `switch` that is the
last statement of a closure, a script or a method: the implicit return applies
to the statement, exactly as it does to an `if` or a colon-form `switch`, so an
unmatched selector leaves the body's value at the return type's default. Use
`return switch (...) {...}`, or assign the result, for the expression rules
(link:https://issues.apache.org/jira/browse/GROOVY-12408[GROOVY-12408]).
...
Arrow arms that need more than one statement use a block and `yield`:

[source,groovy]
----
include::../test/SemanticsTest.groovy[tags=switch_expression_yield,indent=0]
----

Multiple constants may share an arm, and Groovy's usual `isCase` matching
(Class, regex, Collection, Closure) is available in both the statement and
the expression forms. Under static compilation, when every case label is an
`int`, `String` or enum constant, the compiler emits `tableswitch` /
`lookupswitch` bytecode in the same style as `javac`.
...
[NOTE]
====
*Closure, lambda, and AIC capture of for-in variables (GROOVY-11792).*
When a for-in loop variable is shared with a deferred closure, lambda, or
anonymous inner class, each iteration uses a fresh binding so the capture
observes the value from the iteration that created it (similar in spirit to
Java enhanced-for / lambda capture for deferred use). Assignments to the loop
variable within the same iteration remain visible to captures created in that
iteration. Classic `for` / `while` loops are unchanged. To restore the
historical single shared binding (every capture sees the final loop value), set
system property `groovy.forin.per.iteration.capture=false` or call
`CompilerConfiguration.setForInPerIterationCaptureEnabled(false)` (language-compatibility
switch, independent of optimization options including `"all" -> false`).
====
...
[source,groovy]
----
include::../test/semantics/LabelsTest.groovy[tags=labels,indent=0]
----
...
syntax tree (AST) so it is possible for an AST transformation to use that information to perform transformations over the code, hence
leading to different semantics. This is in particular what the http://spockframework.github.io/spock/docs/current/index.html[Spock Framework]
does to make testing easier.

It is possible to use labels in the `break` instruction as a target for jump, as in the next example:
...
`&`, mirroring Java's
https://docs.oracle.com/javase/specs/jls/se21/html/jls-15.html#jls-15.16[JLS §15.16]
...
The same syntax works in the `as` form, with parentheses around the intersection:
...
[[classtag-annotation]]
===== The `@ClassTag` annotation

On the JVM, generic type arguments are erased at runtime, so a method that needs the runtime class of a type
argument must take an explicit `Class` token, even though that token repeats a type the compiler already knows.
The `groovy.transform.stc.ClassTag` parameter annotation (incubating since Groovy 6) marks such a token as
_compiler-supplied_: under `@TypeChecked` or `@CompileStatic`, a call that omits the token can select the tagged
overload, and the type checker synthesises the `X.class` argument from the receiver's type argument. The
`asChecked` methods from the GDK declare their token this way:

[source,groovy]
----
include::../test/typing/ClassTagSpecTest.groovy[tags=classtag_additive,indent=0]
----
<1> there is no token-less `asChecked()`: the type checker selects `asChecked(List<T>, Class<T>)` and injects
`String.class`, reified from the receiver's type argument
<2> the checked view rejects ill-typed elements at runtime, exactly as if `asChecked(String)` had been written

Any API can declare tagged parameters; each `@ClassTag` parameter is filled at its declared position and the
supplied arguments map onto the remaining parameters in order:

[source,groovy]
----
include::../test/typing/ClassTagSpecTest.groovy[tags=classtag_declare,indent=0]
----
<1> the token parameter repeats the receiver's type argument `T`, so it is marked compiler-supplied
<2> equivalent to `registry.create('first', StringBuilder)`: the token is reified from `Registry<StringBuilder>`

Selection is normally _additive_: the tagged overload is only chosen when the call as written matches no method,
so the meaning of existing code never changes. An API author may go further and declare a tagged overload with
`@ClassTag(preempt=true)`, allowing it to _preempt_ a token-less overload declared by the same class: existing
calls are transparently upgraded to the checked variant. Since Groovy 6, the checked `withDefault` variants
declare this (their lenient sibling could silently corrupt a typed map's key set, see
link:https://issues.apache.org/jira/browse/GROOVY-11807[GROOVY-11807]):

[source,groovy]
----
include::../test/typing/ClassTagSpecTest.groovy[tags=classtag_preempt,indent=0]
----
<1> under static compilation the call binds `withDefault(Map<K,V>, Class<K>, Class<V>, Closure)` with
`String.class` and `Integer.class` injected; the returned map rejects ill-typed keys and values
<2> reads behave as before for well-typed keys

Preemption is deliberately constrained by three independent controls:

* _intent_: only an overload explicitly marked `@ClassTag(preempt=true)` may displace a token-less one;
an unmarked tag is strictly additive;
* _containment_: a tagged overload can only preempt a token-less overload declared by the same class (or, for
instance methods, the same class hierarchy) — adding a jar to the compile classpath can never re-route calls
to methods it does not own;
* _consent_: the consuming build may veto all preemption:

[source,groovy]
----
include::../test/typing/ClassTagSpecTest.groovy[tags=classtag_disable,indent=0]
----
<1> also available without a configuration script via the `groovy.classtag.preemption.disable` system property;
code compiled with the veto behaves exactly as it did before any library declared preemption

Some boundaries worth knowing:

* injection is a static-checking feature: dynamic Groovy ignores the annotation, so dynamic code passes the
`Class` token explicitly;
* the receiver's type argument must be statically known — `def`, raw and wildcard receivers synthesise no token —
and only the erased class is reified (`List<List<String>>` yields `List.class`);
* when every token would erase to `Object` (for example a `List<Object>` receiver), no token is synthesised:
a checked `Object` view could never reject anything;
* passing the `Class` explicitly always works, in every mode.
