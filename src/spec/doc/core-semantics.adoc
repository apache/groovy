...
of a method, closure or script body is that body's implicit return value: it
keeps its value, but an unmatched selector yields `null` rather than throwing
and no exhaustiveness is required. Use `return switch (...) {...}` there for
the strict expression rules.
