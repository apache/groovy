...
complete enum) is a compile-time error. From Groovy 6 the compiler keeps the
expression as first-class AST (`SwitchExpression` / `YieldStatement`) rather
than rewriting it to a closure around a switch statement.
