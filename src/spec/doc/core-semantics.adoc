...
and simply does nothing when unmatched. A `switch` that is the last statement
of a closure, a script or a method that returns a value is that body's implicit
return value: it keeps its value, but an unmatched selector yields `null`
rather than throwing and no exhaustiveness is required. Use
`return switch (...) {...}` there for the strict expression rules.
