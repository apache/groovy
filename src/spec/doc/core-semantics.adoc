...
By default, Groovy doesn't perform any kind of type checking at compile time. Since it is primarily a dynamic language,
no such thing can normally be done. Let' illustrate why in the following example:
...
The code above doesn't throw any error. In Java, this would typically fail at compile time. However, in Groovy, it will
not fail, neither at compile time *nor* runtime. In fact, to make this work at runtime, *one* possibility is to rely
on runtime metaprogramming. So just adding this line after the declaration of the `Person` class is enough:
