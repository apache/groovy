...
v1.0, 2014-07-24 updated 11 Sept. 2014
:toc: left
...
<h4>4. The syntax (and LAF) of asciidoctor 0.1.5 differs from 0.1.4 in several ways, do we need to rejig existing *.adoc files to use the revised syntax ?</h4>
...
=== Notes About GitHub  icon:github[link="https://github.com/", window="_blank"]

At *[blue]#Groovy#*, we keep https://github.com/groovy/groovy-core[our _source code_ here, window="_blank"].

==== GitHub Information icon:github[link="https://github.com/", window="_blank"]
...
<li>make a copy ( FORK ) of it into your own online GitHub account
 ...
 <li>provide test cases to confirm your documentation
 <li>test and build everything
...
==== Visit Groovy Core Repository on GitHub icon:github[link="https://github.com/groovy/groovy-core", window="_blank"]
...
Ok, let's look for the *groovy-core* repository. In the field named +Search or type a command+ enter *groovy-core* then click +Search+. You should see a repository named *groovy/groovy-core* so click that to go to home to [blue]#Groovy#.
...
Fork the https://github.com/groovy/groovy-core[groovy/groovy-core] repository on *GitHub* to your own *GitHub* account. Click the +Fork+ button while on the *groovy/groovy-core* panel. When you look at your *GitHub* home page, you'll see the list of your forked repos and any new repos you make.
...
To make sure you can build and test your [blue]#Groovy# version, try to build the full [blue]#Groovy# document suite. 
...
You can check your local copy to see if it looks good. Using the example above, you should have an *index.html* page located in +file://localhost/media/martin/LINKS/groovy-core/groovy-core/target/asciidoc/index.html+ though your actual address of the index.html file will differ a little. Enter your own address as a browser address, then you should see this :

.Groovy Document Index
image::assets/img/console3.png[Groovy Document Index, align="center"] 

As you update documents, you may need to see how it looks. Following the previous procedure, you can do it using the +./gradlew asciidocAll+ command, then open Groovy Index page and navigate to the topic you've updated.
...
So now you know how and where to look, you can review our [blue]#Groovy# documents depending on your area of interest. Find a subject title marked something like +== Operator overloading (TBD)+ in one of our *.adoc* files, or whatever takes your fancy. 
...
To let us know you're working on a topic, it's useful to put your id, like nickname, date, etc., next to your topic. Edit the *.adoc* file to insert your id. After that, save the document, commit the change and send ( *PUSH* ) it back to your own *GitHub* copy of [blue]#Groovy#. Finally, you issue a request to the [blue]#Groovy# admins icon:user[] to merge your amendment back into the [blue]#Groovy# code base from your *GitHub* account. 
...
Here's a demo we did earlier. Notice the line +==== MarkupBuilder (TBD) - assigned to JNorthr 1July14+ assigned to one of our documentation team ?

You need to do the same, to an +.adoc+ document in your own local system. Look in a folder named something like +./groovy-core/groovy-core/src/spec/doc+ to see where your topic has been discussed. Then edit it, adding your id and save as plain text format using the *UTF-8* character set.
...
would only insert source between the +BuilderSupportTest_add_node_with_value_object4+ tag start and end as follows : 
...
Asciidoc tags are simply remarks / comments and so are invisible at build time. They're declared in pairs as a tag set. The *tag* keyword preceeds the tag name +BuilderSupportTest_add_node_with_value_object4+ followed by the asciidoc required tag ending of +[]+
...
Review the https://github.com/groovy/groovy-core/tree/master/src/spec/test[Test Case] folder. There may already be tests that cover your area of interest. Some were written before tag standards  came into existence. It might be as simple as adding asciidoc tags around a test method.
...
The *assert* statement should have a remark before it. It should explain say why the test should succeed - *not* why it would fail icon:exclamation[]
...
It's not critical to have samples of test stuff. Plenty of great documents do not require any reference to tests or the content may not even be about code, logic, tests, scripts.
...
Follow guidelines for version 0.1.5 of Asciidoctor.
...
Try to keep the size of images small rather than use a very large image then ask asciidoctor to re-scale it smaller. This improves page load times for end users.
...
+git commit -am \'put a nice clear note here about what your did'+
