...
Like JavaScript's `Promise.race()` — settles as soon as the first task
settles, returning its result _or_ propagating its error. A task that fails
first therefore fails the whole `any`, even if a later task would have
succeeded (use `first` if you want the first _success_ instead). Useful for
fallback patterns or latency-sensitive code:
...
| `Awaitable.any`
| First task settles (success or failure)
| Propagates the first settlement's result or error (JS `Promise.race`)
...
NOTE: `all` and `allSettled` accept an empty source list and complete with an
empty result. `any` and `first` require at least one source (and reject `null`
elements), throwing `IllegalArgumentException` otherwise — a "race" or "first
success" with no participants has no meaningful result.
