...
Regular expression evaluation is another way for untrusted input to consume unbounded CPU.
Java's regex engine has no native timeout, so a pattern subject to _catastrophic backtracking_
can effectively hang a thread when fed adversarial input -- a Regular Expression Denial of
Service (ReDoS). Thread interruption doesn't help here: the regex engine never checks the
interrupt flag, so even `@ThreadInterrupt` or `@TimedInterrupt` cannot stop a runaway match --
the hang occurs inside a single JDK library call, not in user code where those transforms
insert their checks.

`@SafeRegex` addresses this for the regex operators. Within the annotated scope, find (`=~`)
and match (`==~`) expressions are rewritten at compile time to deadline-guarded
gapi:groovy.util.regex.RegexGuard[] calls which throw a
gapi:groovy.util.regex.RegexTimeoutException[] if an evaluation exceeds the configured timeout.
Matching semantics are otherwise unchanged:

[source,groovy]
----
include::../test/SaferScriptingASTTransformsTest.groovy[tags=saferegex_handler,indent=0]
----

The `check` method is rewritten to the equivalent of:

[source,groovy]
----
include::../test/SaferScriptingASTTransformsTest.groovy[tags=saferegex_handler_equiv,indent=0]
----

Well-behaved evaluations are unaffected, while a runaway one is cut off shortly after the
deadline:

[source,groovy]
----
include::../test/SaferScriptingASTTransformsTest.groovy[tags=saferegex_control,indent=0]
----

The annotation may be placed on a class, method, or constructor. It may also be placed on a
field or local variable declaration, in which case only regex operators lexically within the
initializer expression are guarded (a guarded field initializer runs, as usual, in the
constructor, or in the static initializer for a static field, where a timeout surfaces as an
`ExceptionInInitializerError` whose cause is the `RegexTimeoutException`).

`@SafeRegex` supports one option:

[cols="1,1,2,3a",options="header"]
|=======================================================================
|Attribute|Default value|Description|Example
|millis|1000|The timeout in milliseconds applied to each guarded regex evaluation within the annotated scope. Must be a positive constant.|
[source,groovy]
----
...
Only regex operators lexically visible within the annotated scope are rewritten. Regex
evaluation via method calls such as `String#matches`, `replaceAll` or `split`, or occurring in
code called from the annotated scope, is not guarded by this transform; for those, use
gapi:groovy.util.regex.RegexGuard[] explicitly -- its `guard(timeout) { ... }` form guards all
regex evaluations reached through Groovy's runtime for the dynamic extent of a closure. See the
link:core-operators.html#_guarding_against_runaway_regular_expressions[operator documentation]
for details.
...
[[transforms-static-compilation]]
==== Transformations and static compilation

The phase advice above is about convenience. If classes using your
transformation may also use `@TypeChecked` or `@CompileStatic` — which for a
transformation shipped by a library or framework is out of your hands — the
phase you pick additionally decides whether the type checker sees the code you
generate at all.

Static type checking runs during instruction selection. Code that your
transformation adds in an earlier phase is visited by the type checker exactly
like code the user wrote: it is checked, and under static compilation every
method call and property access in it is annotated with the resolution
metadata the bytecode writers depend on, chiefly
`StaticTypesMarker.DIRECT_METHOD_CALL_TARGET` (the resolved method) and
`StaticTypesMarker.INFERRED_TYPE` (see
gapi:org.codehaus.groovy.transform.stc.StaticTypesMarker[]). Code injected
during or after instruction selection is never visited by the type checker.
Dynamic Groovy and `@TypeChecked` classes do not care, because their bytecode
dispatches dynamically either way — but under `@CompileStatic` the static
writers have nothing to link against. The result is code that works
dynamically and type-checked yet breaks statically compiled, typically with
"Target method for method call expression hasn't been set", a
`VerifyError`, or a "Cannot find matching method" error that only appears
under `@CompileStatic`. If users of your transformation report errors like
these, check what phase the generating code runs in before suspecting their
code.

The safe pattern is therefore to generate code before instruction selection —
usually semantic analysis for local transformations — and let the type checker
annotate it for you. If your transformation must run late, the options are to
mark the generated methods with `@CompileDynamic` (or generate calls through
the dynamic runtime deliberately), or to attach the required metadata
yourself; the latter means hand-resolving targets the way the type checker
would, which is expert territory for the same reasons described in
<<{core-semantics}#ast-xform-as-extension,Transforming the AST in an extension>>.

TIP: The mirror image of this problem — a type checking *extension* that
vouches for dynamic code under `@TypeChecked`, leaving the static compiler
nothing to link against — is covered in the type checking extensions
documentation: prefer `makeDynamic` (see
<<{core-semantics}#mixed-mode,mixed mode compilation>>) or resolve a real
method target instead of simply setting `handled`.
...
===== Relocating code the user wrote

Some annotations take a closure as a member, and that closure is code written by the user of
your transform. Transforms routinely move such code somewhere else — `@TupleConstructor(pre={...})`
inlines it into the constructor it generates, for example — and several then replace the
annotation member with an empty closure so that the type checker does not also try to check it
in place.

That is fine, provided the relocated code ends up somewhere in the AST and keeps the source
position it had in the original source. Anything inspecting the AST at compile time — IDE
tooling, static analysis, other transforms, a
gapi:org.codehaus.groovy.control.customizers.CompilationCustomizer[] — tells code the user wrote
apart from code the compiler generated by its source position, because generated nodes carry
none. Moving user code somewhere only your own transform can find it, such as node metadata,
makes it invisible to all of them.

If your transform cannot do that, because it evaluates the user's code during compilation or
takes it out of the AST altogether, then provide a documented system property which turns the
transform off, so a build compiling source it does not control can decline the behaviour.
`@Grab` does this with `groovy.grape.enable`, and `@ASTTest` with `groovy.asttest.enable`; both
default to `true`.
