...
`META-INF/groovy/org.codehaus.groovy.transform.ASTTransformation` with a line with the name of the
...
. create the `org.codehaus.groovy.transform.ASTTransformation` descriptor inside the `META-INF/groovy` directory (`META-INF/services` is supported for backwards compatibility but not recommended)
...
[source,groovy]
.META-INF/groovy/org.codehaus.groovy.transform.ASTTransformation
----
