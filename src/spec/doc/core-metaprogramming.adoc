...
numerous properties. For example, annotating the `Person` class like below will:
* automatically implement the `Comparable` interface
* automatically generate a `compareTo` method with an implementation based on the natural ordering of the classes `first`, `last` and `born` properties
* automatically generate three comparator methods `comparatorByFirst`, `comparatorByLast` and `comparatorByBorn`.
...
----
As an example of a comparator method, the generated `comparatorByFirst` method will look like this:
[source,groovy]
----
...
----
It will have a generated `compareTo` method like this:
[source,groovy]
----
