...
The annotation will only work if the method ends with a single recursive call, i.e. exact same method again.
Only non-void methods are handled (void calls fails compilation).
If you have method overloading, this can trick the compiler, and some  non-tail recursive calls are treated as tail recursive.
