...
Groovy provides `execute` and `toProcessBuilder` methods as two mechanisms to execute external processes.
...
The `execute()` and `start()` methods return a `java.lang.Process` instance, and
`waitForResult()` returns a `ProcessResult` capturing stdout, stderr, and the exit code.
