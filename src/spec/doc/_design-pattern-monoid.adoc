...
Here is how we might code the previous example using Groovy's
parallel collections:

[source,groovy]
----
include::../test/DesignPatternsTest.groovy[tags=monoids_parallel,indent=0]
----
...
We can even do a parallel version:

[source,groovy]
----
include::../test/DesignPatternsTest.groovy[tags=monoids_average_split_parallel,indent=0]
----

Here, we were using the built-in `sum()` method (and `sumParallel()` for the parallel example),
...
[source,groovy]
----
include::../test/DesignPatternsTest.groovy[tags=monoids_average_reworked_parallel,indent=0]
----
