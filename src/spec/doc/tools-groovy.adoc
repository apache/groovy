...
| -l <[host:]port> | | listen on a port and process inbound lines (default: 1960); binds the loopback address unless a host is given | groovy -l 0.0.0.0:1960 -e "line.reverse()"
...
NOTE: When no classpath is specified, `groovy` uses the current directory, the same as
the `java` interpreter. When you supply a classpath with `-cp`/`-classpath` or the `CLASSPATH`
environment variable, it is used as-is and the current directory is not added for you; add `.`
to your classpath explicitly if you also need it. This applies to the command-line tools
generally (`groovyc`, `groovysh`, and so on).
...
`socket`. The listening socket is bound to the loopback address, so only clients on
the same machine can connect. Supply a host to listen more widely -- `0.0.0.0:1960`
for every interface, or a single address to pick one. The script is read once, when the
listener starts, and recompiled for each connection so that one connection cannot leave
state behind for the next; editing the file has no effect until the listener is restarted.

WARNING: A listener bound beyond loopback accepts connections from any peer that can
reach the port. Groovy performs no authentication and the connection is not encrypted,
so the script itself owns whatever checking is needed.
...
# Show Grape provider discovery details (or comment out for quieter operation)
