...
The `-l` option runs the given script once for every line a client sends, with the
...
for every interface, or a single address to pick one. The script is re-read for each
