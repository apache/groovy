...
uses dynamically, `java.lang.reflect.Proxy` instances for closures coerced to interfaces, and
serialization depend on the application, and the native-image agent remains the way to record them.
