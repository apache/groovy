...
challenges for the DSL implementor, such as securing execution of user
...
extensions (including plain old java code), the recommanded way is to
...
corresponding methoda list with exactly one element, saying that there’s
...
Gives the
developper the ability to handle incorrect assignments. This is for
...
method node. If you combine this with deferred checks, you can acheive
...
complex situations like mutiple builders in the same file or embedded
...
you from modifying the AST. However, we do not recommand you to do so, unless you are an advanced AST transformation
designer and well aware of the compiler internals:

* First of all, you would explicitely break the contract of type checking, which is to annotate,
...
yourself, which can easily become complex. That's why we do not recommand to go that way if you are beginning with
