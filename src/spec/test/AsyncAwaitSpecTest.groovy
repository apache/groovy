...
// end::exception_multiple_tasks[]
    ...
    // =========================================================================
    // 8. Utility methods — await Awaitable.all, Awaitable.any, Awaitable.allSettled, delay
    // =========================================================================
...
assert await(parallelCompute()) == [100, 400, 900]
    ...
    @Test
    void testMultiArgAwaitUnparenthesized() {
        assertScript '''
// tag::multi_arg_await_no_paren[]
import groovy.concurrent.Awaitable

async parallelFetch() {
    def userTask   = Awaitable.go { 'user-42' }
    def ordersTask = Awaitable.go { ['order-1', 'order-2'] }

    // Unparenthesized multi-arg await — no parentheses needed
    def (user, orders) = await userTask, ordersTask
    return [user: user, orders: orders]
}
// end::multi_arg_await_no_paren[]

def result = await parallelFetch()
assert result.user == 'user-42'
assert result.orders == ['order-1', 'order-2']
        '''
    }
    ...
    @Test
    void testMultiArgAwaitAllFormsEquivalent() {
        assertScript '''
// tag::multi_arg_await_all_forms[]
import groovy.concurrent.Awaitable

async viaAwaitAll() {
    def a = Awaitable.of(10)
    def b = Awaitable.of(20)
    // Explicit Awaitable.all — most verbose
    return await Awaitable.all(a, b)
}

async viaParenthesizedAwait() {
    def a = Awaitable.of(10)
    def b = Awaitable.of(20)
    // Parenthesized multi-arg — syntactic sugar for Awaitable.all
    return await(a, b)
}

async viaUnparenthesizedAwait() {
    def a = Awaitable.of(10)
    def b = Awaitable.of(20)
    // Unparenthesized multi-arg — most concise
    return await a, b
}
// end::multi_arg_await_all_forms[]

assert await(viaAwaitAll()) == [10, 20]
assert await(viaParenthesizedAwait()) == [10, 20]
assert await(viaUnparenthesizedAwait()) == [10, 20]
        '''
    }

    @Test
    void testMultiArgAwaitDisambiguationInCollections() {
        assertScript '''
// tag::multi_arg_await_disambiguation[]
import groovy.concurrent.Awaitable
...
async demo() {
    ...
    // CORRECT — parentheses disambiguate each await in a list literal
    def list = [await(a), await(b), await(c)]

    // CORRECT — parentheses disambiguate each await in a map literal
    def nameTask = Awaitable.of('Alice')
    def ageTask  = Awaitable.of(30)
    def map = [name: await(nameTask), age: await(ageTask)]

    return [list: list, map: map]
}
// end::multi_arg_await_disambiguation[]

def r = await demo()
assert r.list == [1, 2, 3]
assert r.map == [name: 'Alice', age: 30]
        '''
    }

    @Test
    void testAwaitableFirstCombinatorBasic() {
        assertScript '''
// tag::first_combinator_basic[]
import groovy.concurrent.Awaitable

async hedgedRequest() {
    // Simulate multiple sources with varying latency
    def primary  = Awaitable.go { Thread.sleep(200); 'primary-result' }
    def fallback = Awaitable.of('fallback-result')

    // first() returns the first *successful* result,
    // ignoring individual failures — like JavaScript's Promise.any()
    return await Awaitable.first(primary, fallback)
}
// end::first_combinator_basic[]

assert await(hedgedRequest()) == 'fallback-result'
        '''
    }

    @Test
    void testAwaitableFirstWithGracefulDegradation() {
        assertScript '''
// tag::first_combinator_degradation[]
import groovy.concurrent.Awaitable
...
async fetchWithFallback() {
    ...
    def cachedValue = Awaitable.go { Thread.sleep(30); 'cached-result' }

    // Even though two sources fail, first() returns the one that succeeds
    return await Awaitable.first(primaryDb, replicaDb, cachedValue)
}
// end::first_combinator_degradation[]

assert await(fetchWithFallback()) == 'cached-result'
        '''
    }

    @Test
    void testAwaitableFirstAllFail() {
        assertScript '''
// tag::first_combinator_all_fail[]
import groovy.concurrent.Awaitable
...
async allFail() {
    ...
    try {
        ...
        // When ALL sources fail, first() throws with all errors as suppressed
        assert e.message.contains('All 2 sources failed')
        assert e.suppressed.length == 2
        return 'caught-aggregate'
    }
}
// end::first_combinator_all_fail[]

assert await(allFail()) == 'caught-aggregate'
        '''
    }

    @Test
    void testCombinatorQuickReference() {
        assertScript '''
// tag::combinator_quick_reference[]
import groovy.concurrent.Awaitable

async combinatorDemo() {
    def fast = Awaitable.of(1)
    def slow = Awaitable.go { Thread.sleep(50); 2 }

    // all() — wait for ALL, fail on first error (Promise.all)
    def allResults = await Awaitable.all(fast, slow)
    assert allResults == [1, 2]

    // any() — first to complete, success OR failure (Promise.race)
    def anyResult = await Awaitable.any(Awaitable.of('a'), Awaitable.of('b'))
    assert anyResult in ['a', 'b']

    // first() — first SUCCESSFUL result, ignore failures (Promise.any)
    def failing = Awaitable.failed(new RuntimeException('skip'))
    def ok = Awaitable.of('winner')
    def firstResult = await Awaitable.first(failing, ok)
    assert firstResult == 'winner'

    // allSettled() — wait for ALL, never throws (Promise.allSettled)
    def settled = await Awaitable.allSettled(Awaitable.of(1), Awaitable.failed(new RuntimeException('e')))
    assert settled[0].success && settled[0].value == 1
    assert settled[1].failure && settled[1].error.message == 'e'

    return 'all-combinators-ok'
}
// end::combinator_quick_reference[]

assert await(combinatorDemo()) == 'all-combinators-ok'
        '''
    }
}
