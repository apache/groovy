...
final class AsyncAwaitSpecTest {
    ...
    void testJdkIntegration() {
        ...
        // setup: temp file containing a password
        def etcPassword = Files.createTempFile('passwords', '.txt')
        Files.writeString(etcPassword, 'admin:sEcrEt\\nroot:P4ssw0rd')

        // setup: fake bank HTTP server
        def server = HttpServer.create(new InetSocketAddress('127.0.0.1', 0), 0)
        server.createContext('/') { exchange ->
            def body = '<html>SUCCESS</html>'
    ...
    @Test
    void testAwaitCompletableFutureNoAmbiguity() {
        // Verifies that await works directly with HttpClient.sendAsync()
        // which returns a CompletableFuture (implements both CompletionStage and Future)
        assertScript '''
        import com.sun.net.httpserver.HttpServer
        import java.nio.charset.StandardCharsets
        ...
        server.start()
        def url = "http://127.0.0.1:${server.address.port}"

        try {
            def result = async {
                def client = HttpClient.newHttpClient()
                def req = HttpRequest.newBuilder().uri(URI.create(url)).build()
