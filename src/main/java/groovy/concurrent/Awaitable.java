 ...
 *   <li>{@link AsyncChannel#create() AsyncChannel.create()} / {@link AsyncChannel#create(int) AsyncChannel.create(n)} — buffered or unbuffered channels</li>
 * </ul>
 * <p>
...
public interface Awaitable<T> {
     ...
     * <p>
     * Inside an {@link AsyncScope}, the new task is automatically attached to
     * the current structured scope — the scope will wait for it on close,
     * and fail-fast cancellation applies.  Outside a scope it behaves like
     * a detached async task.
     * <p>
     * The child task inherits a snapshot of the current
     * {@link groovy.concurrent.AsyncContext AsyncContext}, ensuring that
     * request IDs, tenant IDs, and other execution-scoped metadata
     * propagate automatically.
     *
     * @param closure the task body
     * @param <T>     the result type
     * @return an awaitable representing the spawned task
     * @see AsyncScope#async(groovy.lang.Closure)
     * @since 6.0.0
     */
