 ...
 *   <li>Transform any {@code await(expr)} calls within the method to use {@link org.apache.groovy.runtime.async.AsyncSupport#await AsyncSupport.await}</li>
 ...
 * </pre>
 * <p>
 * For ad-hoc async execution without annotating a method, use the {@code async}
 * closure syntax.  Note that {@code async { ... }} returns a {@code Closure}
 ...
 *     return process(data)
 * }
 * def result = task().get()    // call the closure, then get the result
 * </pre>
 * <p>
 * <b>Custom Executor:</b> By default, tasks run on virtual threads (JDK 21+) or
 * a cached daemon thread pool whose maximum size is controlled by the system
 * property {@code groovy.async.parallelism} (JDK &lt; 21). You can specify a custom
...
public @interface Async {
     ...
     * JDK 21+, or a cached daemon thread pool controlled by the system
