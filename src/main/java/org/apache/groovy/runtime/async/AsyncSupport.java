 ...
 *       (e.g., in generators or nested {@code await} calls).</li>
...
public class AsyncSupport {
    ...
    static {
        ...
        if (!VIRTUAL_THREADS_AVAILABLE) {
            ...
            FALLBACK_EXECUTOR = new java.util.concurrent.ThreadPoolExecutor(
     ...
     */
    public static ArrayDeque<Closure<?>> createDeferScope() {
     ...
     */
    public static void defer(ArrayDeque<Closure<?>> scope, Closure<?> action) {
        scope.push(action);
    }
     ...
     *
     * @param scope the defer scope to execute
     */
    public static void executeDeferScope(ArrayDeque<Closure<?>> scope) {
