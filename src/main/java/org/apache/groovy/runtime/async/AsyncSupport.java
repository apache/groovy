...
public class AsyncSupport {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> Awaitable<T> timeout(Object source, long duration, TimeUnit unit) {
        ...
        TimeoutException te = new TimeoutException(
                "Timed out after " + duration + " " + unit.name().toLowerCase(Locale.ROOT));
        scheduleTimeoutRace(sourceFuture, result, () -> result.completeExceptionally(te), duration, unit);
     ...
     * @param unit the time unit
     * @param <T> the result type
     * @return a new awaitable yielding either the source result or the fallback
     * @throws IllegalArgumentException if {@code source} is {@code null},
     *         {@code duration} is negative, or {@code unit} is {@code null}
     * @since 6.0.0
     */
    @SuppressWarnings("unchecked")
    public static <T> Awaitable<T> timeoutOr(Object source, T fallback, long duration, TimeUnit unit) {
        validateTimeoutArguments(source, duration, unit, "Awaitable.timeoutOr");
        CompletableFuture<T> sourceFuture = (CompletableFuture<T>) toCompletableFuture(source);
        CompletableFuture<T> result = new CompletableFuture<>();
        scheduleTimeoutRace(sourceFuture, result, () -> result.complete(fallback), duration, unit);
        return GroovyPromise.of(result);
    }
