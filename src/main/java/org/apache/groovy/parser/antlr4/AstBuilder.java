...
public class AstBuilder extends GroovyParserBaseVisitor<Object> {
        ...
        try {
            ...
            List<Tuple3<List<Statement>, Boolean, Boolean>> statementInfoList =
                    ctx.switchBlockStatementExpressionGroup().stream()
                            .map(e -> {
                                e.putNodeMetaData(SWITCH_EXPRESSION_VARIABLE_NAME, variableName);
    ...
    public Tuple3<List<Statement>, Boolean, Boolean> visitSwitchBlockStatementExpressionGroup(SwitchBlockStatementExpressionGroupContext ctx) {
        ...
        List<Statement> result = (List<Statement>) ctx.switchExpressionLabel().stream()
                ...
                .reduce(new ArrayList<Statement>(4), (r, e) -> {
                    ...
                    VariableExpression switchExpressionVar = varX((String) ctx.getNodeMetaData(SWITCH_EXPRESSION_VARIABLE_NAME));
    ...
    public Tuple3<Token, List<Expression>, Integer> visitSwitchExpressionLabel(SwitchExpressionLabelContext ctx) {
        ...
        if (asBoolean(ctx.CASE())) {
            ...
            expressionList = expressionList.stream().map(expr -> {
                if (expr instanceof MethodCallExpression) {
                    MethodCallExpression mce = (MethodCallExpression) expr;
                    String methodName = mce.getMethodAsString();
                    if (mce.isImplicitThis() && Character.isUpperCase(methodName.charAt(0))) {
                        List<Expression> argList = ((TupleExpression) mce.getArguments()).getExpressions();
                        boolean allVariableArgs = !argList.isEmpty() && argList.stream()
                                .allMatch(arg -> arg instanceof VariableExpression
                                        && Character.isLowerCase(((VariableExpression) arg).getName().charAt(0)));
                        if (allVariableArgs) {
                            VariableExpression variableExpression = new VariableExpression(methodName);
                            variableExpression.putNodeMetaData(SWITCH_EXPRESSION_DECONSTRUCTED, expr);
            ...
            return tuple(ctx.CASE().getSymbol(), expressionList, acType);
        } else if (asBoolean(ctx.DEFAULT())) {
