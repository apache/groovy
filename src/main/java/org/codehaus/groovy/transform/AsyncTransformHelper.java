...
public final class AsyncTransformHelper {
     ...
     * <br>
     * <b>Multi-argument:</b> when {@code arg} is an {@link ArgumentListExpression}
     * with two or more entries, produces
     * {@code AsyncSupport.await(Awaitable.all(arg1, arg2, ..., argN))},
     * making both {@code await(p1, p2, p3)} and {@code await p1, p2, p3}
     * semantically equivalent to {@code await Awaitable.all(p1, p2, p3)}.
     * <p>
     * The unparenthesized form {@code await a, b, c} is syntactic sugar that
     * works identically to the parenthesized form {@code await(a, b, c)}.
     * When used inside comma-separated contexts (list literals, map literals,
     * or method argument lists), the parenthesized form should be used to
     * avoid ambiguity — e.g. {@code [await(a), await(b)]} rather than
    ...
    public static Expression buildAwaitCall(Expression arg) {
        if (arg instanceof ArgumentListExpression args && args.getExpressions().size() > 1) {
            // Multi-arg: await(p1, p2, ..., pn) → await(Awaitable.all(p1, p2, ..., pn))
