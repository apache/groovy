 ...
 * </ol>
 * <p>
 * Yield-return detection/rewriting and defer scope injection are delegated
 * to {@link AsyncTransformHelper}, which is shared with the ANTLR4 parser's
 * {@code async { ... }} closure handling to avoid code duplication.
 * <p>
...
public class AsyncASTTransformation extends AbstractASTTransformation {
    ...
    public void visit(ASTNode[] nodes, SourceUnit source) {
        ...
        if (executorFieldName != null && !executorFieldName.isEmpty()) {
            ...
            executorExpr = callX(AsyncTransformHelper.ASYNC_SUPPORT_TYPE, "getExecutor",
        ...
        if (hasYieldReturn) {
            ...
            if (isVoid) {
                asyncCall = callX(AsyncTransformHelper.ASYNC_SUPPORT_TYPE,
                        AsyncTransformHelper.EXECUTE_ASYNC_VOID_METHOD, args(closure, executorExpr));
            } else {
                asyncCall = callX(AsyncTransformHelper.ASYNC_SUPPORT_TYPE,
                        AsyncTransformHelper.EXECUTE_ASYNC_METHOD, args(closure, executorExpr));
            }
    ...
    private static class AwaitCallTransformer extends ClassCodeExpressionTransformer {
        ...
        public Expression transform(Expression expr) {
            ...
            if (expr instanceof MethodCallExpression mce
                    && AsyncTransformHelper.AWAIT_METHOD.equals(mce.getMethodAsString())
                    && mce.isImplicitThis()) {
                Expression transformedArgs = mce.getArguments().transformExpression(this);
                StaticMethodCallExpression replacement = callX(
                        AsyncTransformHelper.ASYNC_SUPPORT_TYPE,
                        AsyncTransformHelper.AWAIT_METHOD, transformedArgs);
