 ...
 * <ol>
 *   <li>{@code await expr} (or {@code await(expr)}) calls within the method body are redirected to
 ...
 *       each deferred closure bound to the async context active at registration
 *       time</li>
...
public class AsyncASTTransformation extends AbstractASTTransformation {
    ...
    /**
     * Walks the method body and transforms {@code await expr} (or {@code await(expr)}) calls
     ...
     * methods where {@code await} may be parsed as a regular method call.
     */
