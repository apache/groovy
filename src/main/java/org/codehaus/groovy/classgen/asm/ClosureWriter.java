...
public class ClosureWriter {
    ...
    // Emitted bytecode references only the central bytecode-facing surface (GROOVY-12151 review):
    // bootstraps live on IndyInterface, the adapter factory on ScriptBytecodeAdapter, and the
    // linked dispatcher bundle travels as Object -- GeneratedDispatcher, its Bundle and
    // PackedClosure never appear in generated class files.
    private static final String INDY_INTERFACE_TYPE = "org/codehaus/groovy/vmplugin/v8/IndyInterface";
    private static final String SBA_TYPE = "org/codehaus/groovy/runtime/ScriptBytecodeAdapter";
    ...
    // IndyInterface.packedDispatchers (delegating to GeneratedDispatcher.bootstrap) into one
    // constant bundle, lazily on first adapter creation; typed Object to keep the Bundle type
    ...
    private void writePackedClosure(final ClosureExpression expression) {
        ...
        if (vs != null) {
            for (Iterator<Variable> it = vs.getReferencedLocalVariablesIterator(); it.hasNext(); ) {
                ...
                capturedTypes.put(v.getName(), GenericsUtils.erasure(v.getOriginType()));
        ...
        if (implicitParam) {
            ...
            // A nested closure literal capturing the implicit `it` expects the shared Reference in
            // its constructor, so the hoisted method must Reference-thread the parameter exactly as
            // the generated doCall would: the scope's declared `it` (the variable the nested body
            // actually references) carries the closure-shared flag -- copy it, as
            // createClosureClass does, so CompileStack.init wraps the parameter at method entry.
            Variable itDecl = (vs == null) ? null : vs.getDeclaredVariable("it");
            if (itDecl != null) it.setClosureSharedVariable(itDecl.isClosureSharedVariable());
            closureParams = new Parameter[]{it};
        } else {
            closureParams = (declared != null) ? declared : Parameter.EMPTY_ARRAY;
        }
        ...
        os.push(ClassHelper.OBJECT_TYPE); // the linked bundle, typed Object on the bytecode surface
        ...
        for (int i = 0; i < arity; i++) {
            typesDescriptor.append(BytecodeHelper.getTypeDescription(GenericsUtils.erasure(closureParams[i].getType())));
        }
        typesDescriptor.append(")V");
        mv.visitLdcInsn(new ConstantDynamic("paramTypes", "[Ljava/lang/Class;",
                new Handle(H_INVOKESTATIC, INDY_INTERFACE_TYPE, "packedParamTypes",
        ...
        // and routing through the central adapter class (bundle as Object, result as Closure)
        // keeps PackedClosure and the Bundle type off the emitted-bytecode surface too.
        mv.visitMethodInsn(INVOKESTATIC, SBA_TYPE, "packedClosure",
                "(Ljava/lang/Object;Ljava/lang/Object;ILjava/lang/String;[Ljava/lang/Object;[Ljava/lang/Class;ZZZ)Lgroovy/lang/Closure;", false);
     ...
     * {@code IndyInterface.packedDispatchers} (delegating to {@code GeneratedDispatcher.bootstrap})
