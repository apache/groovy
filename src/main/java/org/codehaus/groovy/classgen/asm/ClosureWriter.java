...
public class ClosureWriter {
     ...
     * automatic path, see {@link #isProvablyDelegateIndependent}) — and it is in a packable position:
    ...
    private PackStrategy chooseStrategy(final ClosureExpression expression) {
        ...
        if (isStaticCompilation()) {
            // Under @CompileStatic packing requires the STC proof of delegate-independence even for
            // the @PackedClosures opt-in: a delegate-resolved body compiles IMPLICIT_RECEIVER
            // expressions against Closure.getDelegate(), which a hoisted method cannot honour.
            triggered = (annotated || SystemUtil.getBooleanSafe("groovy.target.closure.pack"))
                    && isDelegateIndependent(expression);
        } else {
            // Dynamic compilation has no proof; only the annotation's trust assertion packs, and the
    ...
    /** Overridden by the static-compilation closure writer; the type checker's resolution only applies there. */
    ...
    private void writePackedClosure(final ClosureExpression expression) {
        ...
        rebindCaptured(body, allCaptured);  // read-only -> plain param; written -> holder param
        ...
        int mods = ACC_PUBLIC | ACC_SYNTHETIC | (staticContext ? ACC_STATIC : 0);
