...
public final class MultipleAssignmentSupport {
    ...
    public static Object tailRest(final Object rhs, final int fromIndex, final Iterator<?> seq) {
        if (rhs instanceof List) {
            if (fromIndex >= ((List<?>) rhs).size()) return Collections.emptyList();
        ...
        if (rhs != null && rhs.getClass().isArray()) {
            // Match the non-empty branch: ArrayGroovyMethods.getAt(T[], IntRange) returns List<T>,
            // so the empty case must be Collections.emptyList() — not an empty T[] — for typed
            // rest binders like `List<T> *t` to round-trip consistently.
            if (fromIndex >= Array.getLength(rhs)) return Collections.emptyList();
            IntRange range = new IntRange(true, fromIndex, -1);
            return InvokerHelper.invokeMethod(rhs, "getAt", range);
        }
    ...
    public static Object nonTailRestSlice(final Object rhs, final int fromIndex, final int toIndex) {
        ...
        if (size >= 0) {
            ...
            if (fromIndex > effectiveTo || fromIndex >= size || effectiveTo < 0) {
                // Array empty-slice falls through to Collections.emptyList(): the non-empty
                // path delegates to getAt(IntRange) which returns List<T> for arrays, so the
