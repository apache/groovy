...
import org.codehaus.groovy.runtime.metaclass.PatternMetaMethod;
...
public class ExpandoMetaClass extends MetaClassImpl implements GroovyObject {
    ...
    private final LinkedList<DynamicMetaMethod> dynamicMethods = new LinkedList<DynamicMetaMethod> ();
...
//    }
//
    protected DynamicMetaMethod findDynamicMethod(String methodName, Object[] arguments) {
        for (DynamicMetaMethod dyn : dynamicMethods) {
            if (dyn.respondsTo(methodName, arguments))
              return dyn;
        }
        return null;
    }
...
//                    cacheInstanceMethod(key, localMethod);
    ...
    public ExpandoMetaClass define (Closure closure) {
        ...
        GroovyCategorySupport.use(PatternCallCategory.class,closure);
...
//                    cacheStaticMethod(key,metaMethod);
    ...
    private class DefiningClosure extends GroovyObjectSupport {
        ...
        public void pattern (Pattern pattern, Closure closure) {
            ExpandoMetaClass.this.addPatternMethod (pattern, closure);
        }
    ...
    private void addPatternMethod(Pattern pattern, Closure closure) {
        dynamicMethods.addFirst (new PatternMetaMethod(pattern, getTheClass(), closure));
    }


    private static class PatternCallCategory {
        public static void call (Pattern self, Closure closure) {
           Closure owner = (Closure) closure.getOwner();
           InvokerHelper.invokeMethod(owner.getDelegate(), "pattern", new Object[] {self, closure} );
        }
    }
