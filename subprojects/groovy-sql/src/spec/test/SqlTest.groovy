...
'''
    ...
    void testInListExpansion() {
        assertScript '''
            ...
            def driver = 'org.hsqldb.jdbcDriver'
            Sql.withInstance(url, user, password, driver) { sql ->
              sql.execute 'DROP TABLE Author IF EXISTS'
              sql.execute """
              CREATE TABLE Author (
                id          INTEGER GENERATED BY DEFAULT AS IDENTITY,
                firstname   VARCHAR(64),
              ...
              // tag::sql_in_list_positional[]
              def names = ['Dierk', 'Jon']
              def rows = sql.rows('SELECT lastname FROM Author WHERE firstname IN (?)',
                                  [Sql.inList(names)])
              assert rows*.lastname.sort() == ['Koenig', 'Skeet']
              // end::sql_in_list_positional[]

              // tag::sql_in_list_gstring[]
              def picks = ['Dierk', 'Guillaume']
              def rows2 = sql.rows "SELECT lastname FROM Author WHERE firstname IN (${Sql.inList(picks)})"
              assert rows2*.lastname.sort() == ['Koenig', 'Laforge']
              // end::sql_in_list_gstring[]

              // tag::sql_in_list_named[]
              def rows3 = sql.rows('SELECT lastname FROM Author WHERE firstname IN (:firsts)',
                                   [firsts: Sql.inList(['Jon', 'Guillaume'])])
              assert rows3*.lastname.sort() == ['Laforge', 'Skeet']
              // end::sql_in_list_named[]

              // tag::sql_in_list_empty_guard[]
              def selected = []    // e.g. nothing selected on a form
              if (selected) {
                  sql.rows('SELECT lastname FROM Author WHERE firstname IN (?)',
                           [Sql.inList(selected)])
              } else {
                  // skip the query, or build different SQL — Sql.inList([]) throws.
              }
              // end::sql_in_list_empty_guard[]
            }
        '''
    }
