...
----
[NOTE]
`Long.MIN_VALUE` in the `rows` clause is equivalent to SQL's `UNBOUNDED PRECEDING`
...
----
[NOTE]
`Long.MAX_VALUE` in the `rows` clause is equivalent to SQL's `UNBOUNDED FOLLOWING`
