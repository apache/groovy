...
query is rejected at compile time with an error explaining the untranslatable construct. Notable
current restrictions include subqueries, window functions, `groupby` without `into`, and
`orderby`/`limit` within set operation operands; use `groupby ... into g` for aggregation, e.g.
`g.count()`, `g.sum(e -> e.salary)`. A small set of common methods maps to SQL functions, e.g.
`toUpperCase()` to `UPPER(...)` and `length()` to `CHAR_LENGTH(...)`.
