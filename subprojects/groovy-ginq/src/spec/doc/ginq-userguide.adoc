...
NOTE: The set operation keyword must appear on its own line.
Set operations are also available as methods on any `Queryable` result:
`q1.union(q2)`, `q1.intersect(q2)`, `q1.minus(q2)`.
...
A GINQ query is an ordinary (blocking) expression, so you can run it asynchronously using Groovy's
`async`/`await` with no GINQ-specific support: wrap the query in `async { ... }` to launch it, carry on
with other work, then `await` the result when you need it.
[source, groovy]
----
include::../test/org/apache/groovy/ginq/GinqTest.groovy[tags=ginq_async_await,indent=0]
----

=== Customize GINQ

NOTE: *INCUBATING*: the custom code-generator (provider) extension point described here -- the
`provider` and `astWalker` options, the `GinqProvider` registry and the `GinqAstVisitor` SPI --
is still incubating and may change in a future release, even though the rest of GINQ is stable.
...
Alternatively, a code generator can be selected by short name through the `provider` option.
The name is resolved through the `org.apache.groovy.ginq.provider.GinqProvider` SPI, so provider
modules on the compile classpath register themselves automatically. The built-in provider is
`collection`, the default, which executes queries in memory.

=== Executing GINQ against a database

NOTE: *INCUBATING*: the SQL backend described here is provided by the optional `groovy-ginq-sql`
module (GEP-26) and is still incubating.

With the `groovy-ginq-sql` module on the classpath, the same query syntax can be executed against
a relational database by selecting the `native-sql` provider and supplying a `dataSource`, which
may be a `groovy.sql.Sql`, a `javax.sql.DataSource` or a `java.sql.Connection`. Table references
are string literals naming the tables:
[source, groovy]
----
def threshold = 2500
...
Rather than loading rows into memory, the query is translated at compile time into ANSI SQL, e.g.
`SELECT e.name, d.name AS deptName FROM employees e INNER JOIN departments d ON e.deptId = d.id
WHERE e.salary > ?`, and filtering, joining and aggregating are pushed down to the database.
The result rows are `groovy.sql.GroovyRowResult` instances.

Every literal and captured variable in the query becomes a positional bound parameter, so the SQL
text never contains user-supplied values and SQL injection is structurally impossible.

The SQL backend operates in strict mode: an expression either translates cleanly to SQL, or the
query is rejected at compile time with an error explaining the untranslatable construct. The main
current restriction is window functions. Aggregation is supported both in the classic form, e.g.
`groupby e.deptId` with `select e.deptId, count(), sum(e.salary)`, and the `groupby ... into g`
form, e.g. `g.count()`, `g.sum(e -> e.salary)`.

Nested queries translate to SQL subqueries: a nested query as a data source becomes a derived
table, `(...).exists()` becomes `EXISTS (...)`, `in` against a nested query becomes `IN (subquery)`,
and a nested query in an expression position becomes a scalar subquery. Subqueries in expression
positions may be correlated, i.e. reference the aliases of enclosing queries; derived tables
cannot, matching standard SQL.

A set of common methods and operators maps to SQL: `toUpperCase()`/`toLowerCase()` to
`UPPER(...)`/`LOWER(...)`, `trim()` to `TRIM(...)`, `length()` to `CHAR_LENGTH(...)`,
`contains(...)`/`startsWith(...)`/`endsWith(...)` to `LIKE` (with wildcard characters in the
runtime value escaped, so it always matches literally), `substring(...)` to `SUBSTRING(...)`,
`replace(a, b)` to `REPLACE(...)`, string concatenation with `+` to `||`, and ternary
expressions to `CASE WHEN ... END`.

A trailing `orderby`/`limit` on the final operand of a set operation applies to the combined
result, matching SQL semantics; the order keys must match columns of the final `select` clause.
(Note that the collection provider instead applies such an `orderby` to the final operand before
combining.) `orderby`/`limit` within earlier operands are rejected.

Because a `dataSource` is a runtime value, it cannot be supplied via the `@GQ` annotation;
use the `GQ`/`GQL` macro form for database-backed queries.

==== The jooq-sql provider

When `org.jooq:jooq` is also on the compile classpath, the `jooq-sql` provider becomes available.
It supports everything the `native-sql` provider supports but uses jOOQ as a compile-time,
dialect-aware SQL renderer: the `dialect` option names one of jOOQ's `SQLDialect` values and the
generated SQL uses that dialect's syntax and function spellings, e.g. HSQLDB's `limit ? offset ?`
in place of the ANSI `OFFSET ? ROWS FETCH NEXT ? ROWS ONLY`:
[source, groovy]
----
def result = GQL(provider: 'jooq-sql', dialect: 'POSTGRES', dataSource: db) {
    from e in 'employees'
    where e.salary > threshold
...
jOOQ is used at compile time only: execution and results are identical to the `native-sql`
provider (bound positional parameters, `GroovyRowResult` rows), and jOOQ does not need to be
on the runtime classpath. With no `dialect` option, jOOQ's `DEFAULT` dialect is used.
Selecting `provider: 'sql'` auto-detects, preferring `jooq-sql` over `native-sql` when both
are available.
