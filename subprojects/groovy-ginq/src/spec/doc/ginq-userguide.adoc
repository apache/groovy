...
`count`, `min`, `max`, `sum`, `avg`, `median`, `stdev`, `stdevp`, `var`, `varp` and the most powerful function `agg`.
[NOTE]
`count(...)`, `min(...)`, `max(...)`, `avg(...)`, `median(...)`, `stdev(...)`, `stdevp(...)`, `var(...)` and `varp(...)` just operate on non-`null` values,
...
As parallel querying will use a shared thread pool, the following code should be executed to release resources before all GINQ statements are completed, otherwise JVM will not exit. The code will wait util all tasks of theads are completed.
[source, groovy]
----
