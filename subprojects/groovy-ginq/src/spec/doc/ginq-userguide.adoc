...
`lead`, `lag`, `firstValue`, `lastValue`, etc.
...
----
[NOTE]
`null` used as the lower bound of `rows` clause is equivalent to SQL's `UNBOUNDED PRECEDING`
...
----
[NOTE]
`null` used as the upper bound of `rows` clause is equivalent to SQL's `UNBOUNDED FOLLOWING`
