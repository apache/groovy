...
GINQ also supports window functions, nested subqueries, and set operations (`union`, `intersect`, `minus`) — see <<Advanced Topics>> for details.
...
__ONLY ONE__ `from` clause is required in GINQ. Multiple data sources are supported through `from` and the related joins.
