...
a second hover/completion SPI — commands, diagnostics, code actions,
and `afterCompile` are the extension valves. The SPI must not take
`internal` types; commands see only `GroovyLspSession`.
