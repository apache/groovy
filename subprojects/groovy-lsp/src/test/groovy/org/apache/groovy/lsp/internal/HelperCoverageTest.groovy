...
final class HelperCoverageTest {
    ...
    void extensionHostIsolatesThrowingPlugins() {
        ...
        host.configure(new CompilerConfiguration(), CompilerSettings.defaults())
        host.afterCompile(null)
        assert host.extraDiagnostics(null, PositionEncoding.UTF16).isEmpty()
        def doc = new CompiledDocument(
                URI.create('file:///tmp/X.groovy'), 1, 'class X {}', null, null, null)
        assert host.extraDiagnostics(doc, PositionEncoding.UTF16).isEmpty()
        def td = new TextDocument(URI.create('file:///tmp/X.groovy'), 'groovy', 1, 'class X {}')
        assert host.extraCodeActions(td, CompilationSnapshot.EMPTY, [],
                new Range(new Position(0, 0), new Position(0, 1)), PositionEncoding.UTF16).isEmpty()
        ...
        assert ExtensionHost.discover(null).ids().isEmpty()
    ...
    void privateUtilityConstructors() {
        ...
        def ctor = keyType.getDeclaredConstructor(List, boolean, boolean, ClassLoader)
        ctor.accessible = true
