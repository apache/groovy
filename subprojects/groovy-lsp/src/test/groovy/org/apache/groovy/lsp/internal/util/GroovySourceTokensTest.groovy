...
final class GroovySourceTokensTest {
    ...
    @Test
    void shebangAndBrokenNumericLiteralProduceNoCoveringToken() {
        def skipped = '#!only\nclass C {}'
        assert !GroovySourceTokens.tokenize(skipped).isEmpty()
        assert GroovySourceTokens.covering(skipped, 0) == null
        assert GroovySourceTokens.tokenize('#!only').isEmpty()
        assert GroovySourceTokens.covering('#!only', 1) == null
        assert GroovySourceTokens.tokenize('1_').isEmpty()
        assert GroovySourceTokens.covering('1_', 0) == null
    }
}
