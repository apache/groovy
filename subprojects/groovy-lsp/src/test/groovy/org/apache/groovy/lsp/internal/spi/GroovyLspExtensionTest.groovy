...
final class GroovyLspExtensionTest {
    ...
    void unknownCommandStillReturnsNullWithoutPlugins() {
        ...
        assert 'groovy.lsp.stc' in ExtensionHost.discover(GroovyLspExtensionTest.classLoader).ids()
    }

    @Test
    void sessionExposesFoldersTextSettingsAndEncoding() {
        def plugin = new RecordingExtension()
        fixture = new LspFixture(new LanguageServerContext(ExtensionHost.of(plugin)))
        def uri = fixture.open('Hello.groovy', 'class Hello {}\n')
        fixture.server.workspaceService.didChangeConfiguration(new DidChangeConfigurationParams(
                [groovy: [classpath: ['lib/a.jar'], dependencies: 'g:a:1', gdsl: 'gdsl.groovy']]))
        assert plugin.initializedSessions == 1
        def session = fixture.server.context
        assert session.documentText(URI.create(uri))?.contains('class Hello')
        assert session.workspaceFolders() != null
        assert session.extraSetting('dependencies') == 'g:a:1'
        assert session.extraSetting('gdsl') == 'gdsl.groovy'
        assert session.extraSetting(null) == null
        assert session.positionEncoding() in ['utf-16', 'utf-8', 'utf-32']
        assert 'lib/a.jar' in session.classpath()
        assert plugin.configuredWithSession
        assert plugin.lastModule != null
        assert plugin.lastSourceText?.contains('class Hello')
        fixture.close()
        fixture = null
        assert plugin.shutdowns == 1
    }

    @Test
    void throwingIdAndCommandsDoNotAbortHost() {
        def boom = new GroovyLspExtension() {
            String id() { throw new IllegalStateException('id') }
            List<String> commands() { throw new IllegalStateException('commands') }
        }
        def host = ExtensionHost.of(boom)
        assert boom.class.name in host.ids()
        assert host.commands().isEmpty()
    }

    @Test
    void typeCheckedCallIsBoundByDefaultExtension() {
        fixture = new LspFixture()
        def src = '''\
    ...
    private static final class RecordingExtension implements GroovyLspExtension {
        int compiles
        int initializedSessions
        int shutdowns
        boolean configuredWithSession
        ...
        @Override
        void configure(CompilerConfiguration configuration, GroovyLspSession session) {
            configuredWithSession = session != null
        }

        @Override
        void initialized(GroovyLspSession session) {
            initializedSessions++
        }
        ...
        @Override
        List<Diagnostic> extraDiagnostics(URI uri, String sourceText, ModuleNode module) {
            lastModule = module
            lastSourceText = sourceText
            extraDiagnostics(uri, sourceText)
        }
        ...
        @Override
        List<Either<Command, CodeAction>> extraCodeActions(URI uri, String sourceText,
                                                           List<Diagnostic> diagnostics, Range range) {
            lastSourceText = sourceText
            extraCodeActions(uri, diagnostics, range)
        }
