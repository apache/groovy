...
final class PositionsTest {
    ...
    @Test
    void attributeIdentifierRangeAndPositionEdges() {
        def name = new ConstantExpression('secret')
        def attribute = new AttributeExpression(new VariableExpression('this'), name)
        assert Positions.toIdentifierRange(attribute, 'this.@secret', PositionEncoding.UTF16) == null

        def negative = new ASTNode()
        negative.lineNumber = 1
        negative.columnNumber = -5
        negative.lastLineNumber = 1
        negative.lastColumnNumber = 2
        def positive = new ASTNode()
        positive.lineNumber = 1
        positive.columnNumber = 1
        positive.lastLineNumber = 1
        positive.lastColumnNumber = 2
        assert Positions.innerScore(negative) != Positions.innerScore(positive)

        def hello = new ClassNode('Hello', 0, ClassHelper.OBJECT_TYPE)
        hello.lineNumber = 1
        hello.columnNumber = 1
        hello.lastLineNumber = 1
        hello.lastColumnNumber = 20
        assert Positions.findIdentifier(hello, 'class xHello', PositionEncoding.UTF16) == null

        def index = Positions.getDeclaredMethod('indexOfIdentifier', int[], String, int, int)
        index.accessible = true
        assert index.invoke(null, new int[0], '', 0, 0) == -1

        String emoji = 'a🥤b'
        assert Positions.characterToGroovyColumn(emoji, 2, PositionEncoding.UTF16) == 2
        assert Positions.characterToGroovyColumn('©', 1, PositionEncoding.UTF8) == 1
        assert Positions.characterToGroovyColumn('©', 2, PositionEncoding.UTF8) == 2
        assert Positions.characterToGroovyColumn('€', 1, PositionEncoding.UTF8) == 1
        assert Positions.characterToGroovyColumn('€', 3, PositionEncoding.UTF8) == 2
    }
