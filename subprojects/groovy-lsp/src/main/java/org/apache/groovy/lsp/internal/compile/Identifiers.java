...
public final class Identifiers {
    ...
    /**
     * Identifier covering {@code utf16Offset} on {@code line}, including
     * when the caret sits at the start of the word.
     *
     * @param line a source line
     * @param utf16Offset offset in the line
     * @return the identifier, or {@code ""}
     */
    public static String wordAround(final String line, final int utf16Offset) {
        if (line == null || line.isEmpty()) {
            return "";
        }
        int idx = Math.min(Math.max(utf16Offset, 0), line.length());
        if (idx < line.length() && Character.isJavaIdentifierPart(line.codePointAt(idx))) {
            idx += Character.charCount(line.codePointAt(idx));
        }
        int start = idx;
        while (start > 0 && Character.isJavaIdentifierPart(line.codePointBefore(start))) {
            start -= Character.charCount(line.codePointBefore(start));
        }
        int end = idx;
        while (end < line.length() && Character.isJavaIdentifierPart(line.codePointAt(end))) {
