...
/**
 * Core post-pass after {@code SEMANTIC_ANALYSIS}. {@code @TypeChecked} /
 * {@code @CompileStatic} transforms run at {@code INSTRUCTION_SELECTION},
 * which groovy-lsp never reaches; this copies
 * {@code DIRECT_METHOD_CALL_TARGET} onto {@code methodTarget} so hover,
 * rename and parameter-name inlays see a compile-time bind. Dynamic
 * Groovy is left alone.
 */
public final class StaticTypeCheckingPass {
    ...
    private StaticTypeCheckingPass() {
    }

    /**
     * Runs the pass on every module in {@code unit}. Failures are isolated
     * so a bad class does not drop the snapshot.
     *
     * @param unit the compiled unit, possibly {@code null}
     */
    public static void apply(final CompilationUnit unit) {
        if (unit == null) {
            return;
        }
    ...
    static void bindTargets(final ClassNode classNode) {
        if (classNode == null) {
            return;
        }
