...
final class SymbolIdentity {
    ...
    boolean refersTo(final ASTNode node, final AstContext context, final CompilationSnapshot snapshot) {
        if (node == null) {
            return false;
        }
        if (node == declaration) {
        ...
        return switch (kind) {
            ...
            case METHOD -> refersToMethod(node, context, snapshot);
    ...
    private boolean refersToMethod(final ASTNode node, final AstContext context, final CompilationSnapshot snapshot) {
