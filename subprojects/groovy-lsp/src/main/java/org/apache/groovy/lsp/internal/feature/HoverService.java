...
public final class HoverService {
    ...
    public Hover hover(final TextDocument document, final CompilationSnapshot snapshot,
        ...
        if (module != null) {
            ...
            node = AstQuery.nodeAt(module, groovy[0], groovy[1]);
        }
        ...
        if (markdown == null || markdown.isBlank()) {
            markdown = identifierHover(document, position, encoding);
        }
        if (markdown == null || markdown.isBlank()) {
            return null;
        }
    ...
    private static String identifierHover(final TextDocument document, final Position position,
    ...
    private static String renderField(final FieldNode field) {
        ...
        String typeName = type == null ? field.getType().getNameWithoutPackage() : type.getNameWithoutPackage();
        return codeFence(typeName + " " + field.getName()) + docs(field);
    }
    ...
    private static String renderVariable(final VariableExpression variable, final ModuleNode module) {
        ...
        String typeName = type == null ? variable.getType().getNameWithoutPackage() : type.getNameWithoutPackage();
        return codeFence(typeName + " " + variable.getName());
    }
