...
the need is common. The Jackson 3 YAML backend that `groovy-yaml` uses is built on
SnakeYAML Engine, not SnakeYAML, so declare `org.yaml:snakeyaml` as a dependency if you
use this.
