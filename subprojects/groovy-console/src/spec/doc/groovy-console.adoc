...
* You can switch between Light, Dark, and System (if supported by your OS) themes from the
`View -> Theme` menu or by clicking the theme cycle button in the toolbar.
The theme preference is remembered across sessions. The System option
follows your operating system's dark mode setting.

image:{reldir_console}/assets/img/gconsole-dark-theme.png[Dark Theme]
...
`Script` menu
* Clicking the line and column of a compilation error, or a stack-trace
reference to the script being edited, moves the caret to that position
