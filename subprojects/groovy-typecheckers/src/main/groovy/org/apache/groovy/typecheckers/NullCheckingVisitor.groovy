 ...
 * Used by both {@code NullChecker} (annotation-only) and {@code StrictNullChecker} (annotation + flow-sensitive).
 *
...
class NullCheckingVisitor extends CheckingVisitor {
    ...
    NullCheckingVisitor(boolean flowSensitive, MethodNode method, Closure errorReporter) {
        ...
        this.methodNonNull = method.returnType != VOID_TYPE && (hasNonNullAnno(method) || (classNonNullByDefault && !hasNullableAnno(method)))
     ...
     * Called from the DSL's {@code run()} method to set up the {@code afterVisitMethod} callback.
     *
     * @param dsl the type checking DSL instance
     * @param flowSensitive whether to enable flow-sensitive null tracking
     */
    static void install(GroovyTypeCheckingExtensionSupport.TypeCheckingDSL dsl, boolean flowSensitive) {
        dsl.afterVisitMethod { MethodNode method ->
            def reporter = { String msg, node -> dsl.addStaticTypeError(msg, node) }
            method.code?.visit(new NullCheckingVisitor(flowSensitive, method, reporter))
        }
    }
    ...
    void visitDeclarationExpression(DeclarationExpression decl) {
        ...
        if (hasNonNullAnno(ve) && isNullExpr(decl.rightExpression)) {
            reportError("Cannot assign null to @NonNull variable '${ve.name}'", decl)
        }
    ...
    void visitReturnStatement(ReturnStatement statement) {
        ...
        if (methodNonNull) {
            if (isNullExpr(statement.expression)) {
                reportError("Cannot return null from @NonNull method '${method.name}'", statement)
            } else if (isKnownNullable(statement.expression)) {
                reportError("Cannot return @Nullable value from @NonNull method '${method.name}'", statement)
            }
        }
    }
    ...
    private void checkDereference(Expression receiver, Expression context) {
        ...
        if (receiver instanceof VariableExpression) {
            ...
            if (target instanceof AnnotatedNode && hasNullableAnno(target) && !guardedVars.contains(target) && !isMonotonicAndInitialized) {
                reportError("Potential null dereference: '${receiver.name}' is @Nullable", context)
            } else if (flowSensitive && nullableVars.contains(target) && !guardedVars.contains(target)) {
                reportError("Potential null dereference: '${receiver.name}' may be null", context)
            }
            ...
            if (targetMethod instanceof MethodNode && hasNullableAnno(targetMethod)) {
                reportError("Potential null dereference: '${targetMethod.name}()' may return null", context)
            }
        }
    }
    ...
    private void checkMethodArguments(call) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (paramIsNonNull) {
                if (isNullExpr(arg)) {
                    reportError("Cannot pass null to @NonNull parameter '${params[i].name}' of '${target.name}'", call)
                } else if (isKnownNullable(arg)) {
                    reportError("Cannot pass @Nullable value to @NonNull parameter '${params[i].name}' of '${target.name}'", call)
                }
            }
        }
    }
