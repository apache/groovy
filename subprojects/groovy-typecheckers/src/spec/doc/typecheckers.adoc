...
Here is an example
which prints PI (to 2 decimal places), the hex representation of 15 (as two uppercase digits with a leading 0),
and the Boolean `True` (in uppercase).
...
* if we supply a String as the parameter for either
of the first two arguments, we would receive an
`IllegalFormatConversionException` at runtime,
* if we supply the
_leading zero_ flag for the Boolean parameter, we would receive a
