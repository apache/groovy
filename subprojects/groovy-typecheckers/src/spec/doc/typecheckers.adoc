...
* `StrictNullChecker` — *Annotation-based + flow-sensitive*: Includes all checks from
...
code and `StrictNullChecker` to strict code via per-class or per-method `@TypeChecked` annotations.
...
*Ternary and elvis expressions*: in flow-sensitive mode (`StrictNullChecker`), the checker
...
`NullChecker` only flags issues involving annotated code. `StrictNullChecker` adds
...
* dereferencing a variable known to be null through flow analysis (`StrictNullChecker` only)
