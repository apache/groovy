...
Both `XmlParser` and `XmlSlurper` support Groovy's named parameter syntax
for configuration, making it easy to set options without remembering
positional boolean argument order:

[source,groovy]
.XmlSlurper
----
include::../test/UserGuideXmlSlurperTest.groovy[tags=testNamedParameters,indent=0]
----

[source,groovy]
.XmlParser
----
include::../test/UserGuideXmlParserTest.groovy[tags=testNamedParameters,indent=0]
----
...
| Allow DOCTYPE declarations in documents (see <<xml-security>>)
| `false`
...
`XmlParser` additionally supports `trimWhitespace` (default `false`)
to trim whitespace from element text content.

NOTE: Options that affect the underlying SAX parser (`namespaceAware`,
`validating`, `allowDocTypeDeclaration`) must be set before the first
parse call. Setting them afterwards throws an `IllegalStateException`.

TIP: Groovy's XML parsers are secure by default — DOCTYPE declarations,
external entities, billion-laughs payloads and external resource resolution
are all blocked. See <<xml-security>> for details and the relax knobs
available when you legitimately need DTD processing.

include::_xml-security.adoc[leveloffset=+1]
...
Suppose we have an existing XML document and we want a head start on
writing the markup rather than typing it all in? We just
need to use `org.apache.groovy.xml.tools.DomToGroovy` as shown in
the following example:

`DomToGroovy` is an authoring aid. It produces builder code to read and edit,
so the generated source is a starting point rather than a finished script.
The example below evaluates it directly to show that the round trip works;
do that only for documents you control, since evaluating generated source
runs whatever the document produced.
...
on the classpath -- a clear error is thrown if it is absent. Jackson 3
(`tools.jackson.core:jackson-databind`) is used when present, otherwise
Jackson 2 (`com.fasterxml.jackson.core:jackson-databind`); either way an
element with no matching property fails the conversion.
...
The `parseTextAs` method uses `jackson-databind` (not `jackson-dataformat-xml`),
