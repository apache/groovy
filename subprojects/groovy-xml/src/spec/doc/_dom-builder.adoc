...
NOTE: `DOMBuilder.parse(...)` and `DOMBuilder.newInstance()` reject DOCTYPE
declarations by default. The `newInstance(validating, namespaceAware,
allowDocTypeDeclaration)` overload exposes a relax knob when DTD support is
required. See <<{xml-userguide}#xml-security,XML security>> for details.