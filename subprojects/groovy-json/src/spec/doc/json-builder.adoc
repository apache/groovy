...
If you need to customize the generated output you can pass a `JsonOutput.Options` object when creating the `JsonBuilder`:

[source,groovy]
----
include::{rootProjectDir}/subprojects/groovy-json/src/spec/test/json/JsonBuilderTest.groovy[tags=json_builder_options,indent=0]
----
