...
If you need control over the serialized output you can use the `JsonOutput.options()` method.  You can set one or
more options on this builder in order to alter the resulting output.  When you are done setting the options simply
call the `createGenerator()` method in order to get a fully configured instance that will generate output based on
the options selected.

[source,groovy]
----
include::{rootProjectDir}/subprojects/groovy-json/src/spec/test/json/JsonTest.groovy[tags=json_output_options,indent=0]
----

In addition to excluding certain values from being generated you can also register converters in order to transform
a type into any valid JSON value.  Converters are registered for a given type and the closure provided will be called
any time that type or a subtype is encountered.  The first parameter to the closure is an object matching the type and
this parameter is required.  The closure may take an optional second String parameter and this will be set to the key
name if one is value.
