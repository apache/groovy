...
//        assert toJson("/") == '"\\/"'
    ...
    void testGeneratorExcludesNullValues() {
    ...
    void testGeneratorWithCustomDateFormat() {
        def generator = JsonOutput.options()
                .dateFormat('yyyy-MM')
                .createGenerator()
    ...
    void testOptionsDateFormatBadInput() {
        shouldFail(NullPointerException) {
            JsonOutput.options().dateFormat(null)
        }
        shouldFail(IllegalArgumentException) {
            JsonOutput.options().dateFormat('abcde')
        }
        shouldFail(NullPointerException) {
            JsonOutput.options().timezone(null)
        }
    }

    void testGeneratorWithConverters() {
        def generator = JsonOutput.options()
    ...
    void testOptionsConvertersBadInput() {
        shouldFail(NullPointerException) {
            JsonOutput.options().addConverter(null, null)
        }
        shouldFail(NullPointerException) {
            JsonOutput.options().addConverter(Date, null)
        }
        shouldFail(IllegalArgumentException) {
            JsonOutput.options().addConverter(Date, {-> 'no args closure'})
        }
        shouldFail(IllegalArgumentException) {
            JsonOutput.options().addConverter(Date, { UUID obj -> 'mis-matched types'})
        }
        shouldFail(IllegalArgumentException) {
            JsonOutput.options().addConverter(Date, { Date obj, UUID cs -> 'mis-matched types'})
        }
    }

    void testGeneratorExcludesFieldsByName() {
        def generator = JsonOutput.options()
    ...
    void testOptionsExcludeFieldsByNameBadInput() {
        shouldFail(NullPointerException) {
            JsonOutput.options().excludeFieldsByName(null)
        }
    }

    void testOptionsExcludeFieldsByNameShouldIgnoreNulls() {
        def opts = JsonOutput.options()
    ...
    void testGeneratorExcludesFieldsByType() {
        def generator = JsonOutput.options()
    ...
    void testOptionsExcludeFieldsByTypeBadInput() {
        shouldFail(NullPointerException) {
            JsonOutput.options().excludeFieldsByType(null)
        }
    }

    void testOptionsExcludeFieldsByTypeShouldIgnoreNulls() {
        def opts = JsonOutput.options()
