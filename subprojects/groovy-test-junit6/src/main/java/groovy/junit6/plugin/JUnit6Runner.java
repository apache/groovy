...
public class JUnit6Runner implements GroovyRunner {
    ...
    private static final System.Logger LOGGER = System.getLogger(JUnit6Runner.class.getName());
    ...
    private boolean isJupiterAnnotationPresent(Annotation[] annotations, GroovyClassLoader loader) {
        for (Annotation annotation : annotations) {
            ...
            if (isJupiterTestableMetaAnnotationPresent(type, new HashSet<>()) && tryLoadClass(name, loader)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isJupiterTestableMetaAnnotationPresent(Class<? extends Annotation> type, Set<String> seen) {
        for (Annotation annotation : type.getAnnotations()) {
            ...
            if (!seen.contains(annotation.annotationType().getName())) {
                ...
                if (isJupiterTestableMetaAnnotationPresent(annotation.annotationType(), seen)) {
                    return true;
                }
            }
        }
        ...
        try {
            try {
                ...
                LOGGER.log(System.Logger.Level.WARNING,
                        "Required dependency: org.junit.platform:junit-platform-launcher doesn't appear to be on the classpath");
            }
            Class<?> helper = loader.loadClass("groovy.junit6.plugin.GroovyJUnitRunnerHelper");
