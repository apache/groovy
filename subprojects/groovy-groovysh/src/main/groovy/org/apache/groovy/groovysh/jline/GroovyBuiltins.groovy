...
class GroovyBuiltins extends Builtins {
    ...
    private void less(CommandInput input) {
        boolean usingBuffer = input.xargs().size() == 0
        if (usingBuffer) {
            def temp = File.createTempFile('groovysh', '.groovy')
            temp.text = engine.buffer
            input = new CommandInput(input.command(), [temp.absolutePath] as String[], input.terminal(), input.in(), input.out(), input.err())
        }
        try {
    ...
    private void nano(CommandInput input) {
        boolean usingBuffer = input.args().size() == 0
        def temp = null
        if (usingBuffer) {
