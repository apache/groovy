 ...
 * The 'browse' command.
 */
class BrowseCommand extends CommandSupport {
    public static final String COMMAND_NAME = ':inspect'

    BrowseCommand(final Groovysh shell) {
    ...
    protected List<Completer> createCompleters() {
        return [
                new BrowseCommandCompleter(binding),
                null
        ]
    }
    ...
    Object execute(final List<String> args) {
        ...
        if (!subject) {
            ...
            if (headless) {
                io.err.println(render("@|red ERROR:|@ Running in AWT Headless mode, 'inspect' is not available."))
                return
            }
...
/**
 * Completer for the 'browse' command.
 */
class BrowseCommandCompleter extends SimpleCompleter {
    private final Binding binding

    BrowseCommandCompleter(final Binding binding) {
        assert binding
