...
//        }
    ...
    static void main(String[] args) {
        try {
            ...
            def rootURL = Main2.getResource('/nanorc')
            ...
            CommandRegistry builtins = new GroovyBuiltins(workDir, configPath, (String fun) ->
                new ConsoleEngine.WidgetCreator(consoleEngine, fun)
            )
            def extra = new ExtraConsoleCommands(workDir)
            CommandRegistry groovy = new GroovyCommands(scriptEngine, printer)
...
*/
            ...
            def highlighter = new SystemHighlighter(commandHighlighter, argsHighlighter, groovyHighlighter).tap {
                ...
                addFileHighlight("/nano", "/less", "slurp")
                addFileHighlight("/classloader", null, Arrays.asList("-a", "--add"))
