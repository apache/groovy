...
* `extraCommandRegistry` — additional JLine `CommandRegistry`
  instances, appended after groovysh's own registries.
* `prompt` — `Supplier<String>` invoked on each REPL iteration
  (default `{ 'groovy> ' }`).
* `resultHandler` — how values are rendered (default:
  `console.println(result?.toString())`).
* `errorHandler` — how execution failures are rendered (default:
  `SystemRegistry.trace`). `UserInterruptException` and
  `EndOfFileException` stay in `Main` as control flow.
...
include::../test/EmbeddingSpecTest.groovy[tags=embed_options_help,indent=0]
...
`ExtraConsoleCommands` class. `Main` still registers it by default.
