...
* The _and_ operator (|&&) executes the right hand side only if the execution of the left hand side succeeds.
* The _or_ operator (|||) executes the right hand side only if the execution of the left hand side fails.

Where _succeeds_ means executing without throwing an exception and returning
a non-null, non-empty, non-zero result, i.e. `true` according to _Groovy truth_.
...
You can also define your own pipe operators, defined using `/pipe [OPERATOR] [PREFIX] [POSTFIX]`,
as these examples show:

image:assets/img/repl_pipe.png[usage of the /pipe command, width=70%]

There is another less commonly used builtin pipe operator, `|;`, which is used to flip arguments:
