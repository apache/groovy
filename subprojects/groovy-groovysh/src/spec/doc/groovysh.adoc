...
* `onReaderReady` — a `Consumer<LineReader>` called once the reader
  exists, for settings the builder does not model: the continuation
  prompt (`SECONDARY_PROMPT_PATTERN`), key bindings, reader options.
