...
Module imports (JEP 476) are also supported, which import all exported packages from a named module:

[source,jshell]
--------------
groovy> import module java.sql
groovy> Connection.name
===> java.sql.Connection
--------------
...
image:{reldir_groovysh}/assets/img/repl_img_jfreechart.png[Using /img with JFreeChart]
...
groovy> figure = ScatterPlot.of(iris, 'sepallength', 'sepalwidth', 'class', '*' as char).figure()
...
groovy> chart = Chart3DFactory.createPieChart('Top 10 by population (millions)', null, data)
groovy> panel = new Chart3DPanel(chart)
groovy> /img $panel --width=700 --height=420
--------------

image:{reldir_groovysh}/assets/img/repl_img_orson.png[Using /img with Orson Charts]
...
groovy> chart = new XYChartBuilder().title('Mauna Loa annual mean CO2').xAxisTitle('Year').yAxisTitle('CO2 (ppm)').width(600).height(400).build()
groovy> chart.addSeries('CO2', rows.collect { it.Year as int }, rows.collect { it.Mean as double })
...
you can also provide an encoding and a format. If no encoding is given,
...
That content-based fallback *parses*; it never evaluates. It recognises JSON
and otherwise hands back the text unchanged, so a document cannot decide for
itself that it should be run. Evaluating content is a separate, explicit
request: pass `-f GROOVY`, or name a local `.groovy` file.

For a URL, the extension of the URL path selects the format in the same way
as a filename does, but only ever a data format &#8212; `GROOVY` is not inferred
from an address, because the extension is part of the URL you typed while the
bytes are whatever the server chooses to return.

WARNING: `-f GROOVY` against a URL runs whatever that server returns, with your
privileges, in this session. It is not a safer way to evaluate remote content
than the old content sniffing was &#8212; it is the *authorised* way, and the
difference is only that you chose it rather than the payload choosing for you.
Redirects are followed, so the address you typed is not necessarily where the
code came from; `/slurp` prints the URL it was actually served from before
evaluating it. Treat it as you would piping a download straight into a shell, and prefer
fetching the content first (without `-f`) so you can read it before deciding
to run it.
...
| GROOVY | *Evaluates* the content as a Groovy script and returns its result. Selected by `-f GROOVY`,
or by a local `.groovy` file extension. Never selected automatically from content, nor from a URL.
...
[[GroovyShell-Embedding]]
=== Embedding groovysh

Applications that host groovysh — a domain-specific console, for
example — should not copy `Main`. The embedding API is
`Main.start(GroovyshOptions, String[])`, plus constructors on
`GroovyEngine` for compile-time customizers.

Do not put embedding hooks into the binding `Map` accepted by
`Main.start(Map, String[])`. That map is only for variables visible to
evaluated scripts.

==== Compiler configuration

`GroovyEngine` always evaluates with an explicit
`CompilerConfiguration` (`CompilerConfiguration.DEFAULT` when you pass
`null`). Use a *mutable copy* (`new CompilerConfiguration()`), not
`CompilerConfiguration.DEFAULT`, when adding customizers. Typical embedder customizers are
`ImportCustomizer` (star and static-star imports that would otherwise
be prepended as snippets on every evaluation) and
`ASTTransformationCustomizer(ThreadInterrupt)` (so Ctrl+C can stop a
tight loop after `Main` interrupts the execute thread).

[source,groovy]
.Compile-time customizers on the engine
----
include::../test/EmbeddingSpecTest.groovy[tags=embed_engine_config,indent=0]
----

Subclasses that need a different `GroovyShell` can override
`protected GroovyShell createShell(ClassLoader, Binding, CompilerConfiguration)`.
That method is called from the constructor: do not read subclass fields
that are assigned after `super(...)`.

`GroovyEngine.getClassLoader()` is the public accessor for the engine
class loader (the field is `protected`; Java callers cannot read it
without a getter).

[source,groovy]
.The engine class loader
----
include::../test/EmbeddingSpecTest.groovy[tags=embed_classloader,indent=0]
----

==== `GroovyshOptions`

`Main.start(GroovyshOptions, String[])` is the programmatic entry point
that does not call `System.exit`. The options object is built with
`GroovyshOptions.builder()` and is incubating in 7.0.

Where a setting has an equivalent on JLine's `ShellBuilder`, it
deliberately carries the same name and parameter type, so that a
future move of groovysh onto `org.jline.shell` does not break this
API.

Recognized hooks:

* `compilerConfiguration` / `engine` — compile-time customizers, or a
  pre-built `GroovyEngine`. If both are set, the engine wins.
* `bindings` / `binding(name, value)` — variables installed on the
  engine before the REPL starts.
* `groups` — additional JLine `CommandGroup` instances
  (`org.jline.shell`, the undeprecated successor to `CommandRegistry`;
  groovysh adapts them internally). They are resolved *before*
  groovysh's own commands, so a group may override a built-in
  command name.
* `prompt` — `Supplier<String>` invoked on each REPL iteration
  (default `{ 'groovy> ' }`).
* `resultHandler` — how values are rendered, given the session
  `Printer` (default: `printer.println(result?.toString())`).
* `errorHandler` — how execution failures are rendered, given the
  error and groovysh's own renderer to delegate to (default: the
  latter). `UserInterruptException` and `EndOfFileException` stay in
  `Main` as control flow.
* `showBanner` — set `false` to suppress groovysh's banner so the host
  can print its own.
* `rightPrompt` — optional right-hand prompt.
* `terminal` — optional JLine `Terminal`; when omitted, `Main` builds
  one from the CLI flags.
* `historyFile` — where to persist history; when omitted, groovysh's
  own `groovysh_history`.
* `onReaderReady` — called once the reader and the command registry
  are both wired, immediately before the REPL starts, for settings the
  builder does not model: the continuation prompt
  (`SECONDARY_PROMPT_PATTERN`), key bindings, reader options, and
  completion. Takes a `Consumer<LineReader>`, or a
  `BiConsumer<LineReader, GroovySystemRegistry>` when the registry is
  needed too. groovysh has already assigned
  `reader.completer = registry.completer()` by then, so an embedder
  contributing domain completion wraps it:
+
[source,groovy]
----
reader.completer = new AggregateCompleter(registry.completer(), myCompleter)
----
+
`AggregateCompleter` merges candidates, so a completer added that way
supplements groovysh's rather than replacing it. Assigning a completer
of your own replaces it outright. Per-command completion needs none of
this — it comes from `Command.completer()` on a contributed `group`.

[source,groovy]
.Embedding with `GroovyshOptions`
----
include::../test/EmbeddingSpecTest.groovy[tags=embed_options_hooks,indent=0]
----

`Main.start(Map, String[])` remains the binding-only overload and
delegates to `GroovyshOptions`.

==== Extra console commands

`/clear`, `/pwd`, `/cd`, `/ls`, `/cat`, `/grep`, `/head`, `/tail`,
`/wc`, `/sort`, `/date`, `/echo` and `/!` live in the public
`ExtraConsoleCommands` class. It implements JLine's deprecated
`org.jline.console.CommandRegistry`, as groovysh's own registries
still do; prefer `GroovyshOptions.groups` and `org.jline.shell`
for new embedder commands. `Main` still registers it by default.
Embedders that build their own `SystemRegistry` can construct it
directly. The former `Main.ExtraConsoleCommands` nested class remains
as a deprecated subclass for Groovy callers that compiled against it.
