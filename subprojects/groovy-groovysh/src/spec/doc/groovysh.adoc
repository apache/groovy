...
JLine provides a powerful widget system that lets you extend the functionality of its line reader.
...
These are available but not enabled by default. You can enable them using the related
_toggle_ widgets. You can see what key bindings are associated with these widgets by
using the `/keymap` command.
...
image:assets/img/widget1.png[Tailtip]

And here for some code:

image:assets/img/widget3.png[Tailtip]

With the autosuggest widget enabled, you can see suggestions for what to type next
as you type, as seen here:

image:assets/img/widget2.png[Tailtip]
