...
**Applies to:** Groovy 3.0.x, 4.0.x, 5.0.x, 5.1.x,
and 6.0.x (currently RC-1~/dev) as covered by [`SECURITY.md`](.github/SECURITY.md).
...
| `CliBuilder.expandArgumentFiles` | `true` | When `true`, an argv token `@file` expands to the contents of `file` (the javac/picocli `@argfile` convention). The argument vector is developer-supplied and trusted ([§6](#6-assumptions-about-inputs)), so this reads only files the invoker named; set `false` (or call `setExpandArgumentFiles(false)`) to disable. *(documented — verified in `CliBuilderInternal`)* |
| `groovy.servlet.verbose.errors` | `false` | When `false`, `GroovyServlet` sends a generic `500` to the client and writes the exception detail, script path and stack frame only to the servlet/application logs. Set `true` (or override `GroovyServlet.isVerboseErrors()`) to restore the detailed client responses while debugging, at the cost of disclosing internal detail to callers. *(documented — verified in `GroovyServlet`)* |
...
| `YamlSlurper` / `TomlSlurper` / `CsvSlurper` | Bounded *implicitly* by the Jackson runtime's `StreamReadConstraints` (nesting-depth / length caps) — the dependency's defaults, not an explicit Groovy decision. On Jackson 3 these are nesting depth `500` and string length `100,000,000` (Jackson 2's were `1000` and `20,000,000`). An earlier revision also credited "the YAML layer's alias limits"; **no such limit is in play, and none is needed on this path**: the Jackson YAML backend never expands an alias at all, so expansion cost cannot arise. A 10^9-leaf alias bomb parses in 0.1s under a 256MB heap — because each alias arrives as a plain string holding its anchor's name, which is a correctness limitation documented in GROOVY-12358 (upstream jackson-dataformats-text#98, closed as not planned). A future move to a YAML engine that *does* resolve aliases must bring a real alias limit with it. |
| `JsonSlurper` | **Now bounded explicitly (6.0.0+).** Its recursive-descent parsers (`decodeValue` → `decodeJsonObject`/`decodeJsonArray` → `decodeValue`) enforce a Groovy-level nesting-depth cap (`BaseJsonParser`, default `1000`, configurable via `setMaxNestingDepth` or `-Dgroovy.json.maxNestingDepth`), throwing a `JsonException` rather than overflowing the stack. The default matches Jackson 2's `StreamReadConstraints` default; Jackson 3, which the YAML/TOML/CSV slurpers use, lowered its own to `500`. *(documented — GROOVY-12064)* |
...
| The Groovy "shell"/console tools | Developer/operator conveniences | `groovysh`/`groovyConsole` run code with the user's full privileges; they are not multi-tenant or sandboxed. `groovy -l` is the same kind of tool with a socket for an input channel instead of a terminal &mdash; an aid to experimenting with a line-driven script, not an application server. Deploying either as though it were is a choice with consequences ([§10](#10-downstream-responsibilities)). |
| `ConfigSlurper` | A reader for configuration written as Groovy | The name places it beside `JsonSlurper`, `YamlSlurper` and the rest, which map a document to values. This one compiles and runs what it is handed, so a configuration file is code: `parse` on text from elsewhere is `Eval` under another name, and [§3](#3-out-of-scope-explicit-non-goals) applies to it rather than [§8](#8-security-properties-the-project-provides) P3. |
...
| Dependency-verification config that trusts by checksum or by group rather than per-artifact signature — `verify-metadata=false`, group-scoped `<trust>` entries, or an `ignored-key` with a checksum fallback (`gradle/verification-metadata.xml`) | Deliberate, scoped dial-downs, each recorded in-file: metadata enforcement is off while jar integrity still holds via `verify-signatures` + checksums; self-resolved baselines (`org.apache.groovy`, `org.codehaus.groovy`) and the doc toolchain (`org.asciidoctor`, `org.jruby`) are trusted by group; keys on no keyserver are pinned by checksum. The checked-in default is strict — `KNOWN-NON-FINDING` unless a *product or test* artifact is trusted with neither signature nor checksum, which is `VALID-HARDENING` |
| Filename-regex `<trust>` of IDE-only `-sources.jar` / `-javadoc.jar` (`gradle/verification-metadata.xml`) | Ancillary artifacts IDEs resolve during Gradle import; not on compile/runtime classpaths. Product and test jar integrity is unchanged (`verify-signatures` + checksums). `KNOWN-NON-FINDING` unless a *product or test* artifact matches the regex, which is `VALID-HARDENING` |
| A CI canary/test job downloading a pre-release toolchain (the `ea` OpenJDK job in `.github/workflows/groovy-build-test.yml`) | Push-only (`if: github.event_name == 'push'` — pull requests, including fork PRs, do not run this job), read-only `GITHUB_TOKEN`, and test-only: the EA/RC JDK runs the suite but never builds or signs a release artifact (Gradle itself runs on a GA JDK). The tarball URL is resolved at runtime from a pinned `https://jdk.java.net/<N>/` page (build numbers move weekly); the download is host-allowlisted to `download.java.net` over HTTPS and checked against that host's published `.sha256` companion. Same-host companion is transport integrity, not a pin — a compromised java.net can serve a matching pair. Cache keys for this job are isolated from the LTS/additional jobs so a substitute JDK cannot persist grapes or wrapper jars into those pipelines. `KNOWN-NON-FINDING` unless the job gains write/publish permissions, shares caches with a release/LTS job, or a release build consumes the pre-release toolchain |
| Nested-path `copyWith` navigating an object graph by dotted map keys (`x.copyWith('a.b': v)`, `NestedCopyWithSupport`) | The keys are developer-supplied property paths; navigation is confined to the `@Immutable`/`@RecordType` graph — each hop must expose `copyWith(Map)` or a clear closed-domain error is thrown, so it cannot escape into `.class`/`.metaClass`. It reads declared properties only, invokes the fixed `copyWith` (never an attacker-named method), mutates nothing, and delegates leaf assignment to the type's own key-validated `copyWith`. Untrusted keys are a downstream decision ([§3](#3-out-of-scope-explicit-non-goals), [§10](#10-downstream-responsibilities)). `KNOWN-NON-FINDING` |
| String-to-Class coercion resolving a class named by a data string (`x as Class` / `castToClass`) | The application chose to turn a string into class resolution, and only classes already on the classpath are reachable; feeding untrusted data to the coercion is a downstream boundary decision ([§3](#3-out-of-scope-explicit-non-goals), [§10](#10-downstream-responsibilities)). **As of 6.0.0 (GROOVY-12375)** the class is resolved *without* being initialized (no static initializer runs on coercion); on 3.0.x/4.0.x/5.0.x/5.1.x the coercion still initializes the class, but the disposition is unchanged either way. `KNOWN-NON-FINDING` unless a Groovy component itself routes untrusted data into it unrequested |
| A launcher putting the current directory on the classpath (`startGroovy`/`startGroovy.bat`) | The user runs the CLI tool from a directory they chose ([§5](#5-assumptions-about-the-environment) CLI-tools, [§6](#6-assumptions-about-inputs)). A bare invocation falls back to the current directory, the same as `java`; an explicit `-cp`/`-classpath` or `CLASSPATH` is honoured as-is with no implicit `.`. `KNOWN-NON-FINDING` unless a tool adds `.` to an explicitly-supplied classpath |
