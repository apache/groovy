...
#
#
# verify-binary-distribution.sh - Verify a downloaded Groovy binary distribution.
#
# Expects the download directory to contain SVN_KEYS (the Groovy project KEYS
# file) and a binary/ subdirectory with the zip, .asc, and .sha256 files, as
...
#
# Usage:
#   verify-binary-distribution.sh <version> [download-dir]
#
...
if [ -z "${ZIP_FILE}" ]; then
  echo "Error: Could not find apache-groovy-binary-${VERSION}.zip in ${DOWNLOAD_LOCATION}/binary"
  exit 1
fi

echo "Verifying checksum..."
EXPECTED_HASH=$(cat apache-groovy-binary-${VERSION}.zip.sha256 | tr -d '\r\n')
ACTUAL_HASH=$(shasum -a 256 apache-groovy-binary-${VERSION}.zip | awk '{print $1}')
...
gpg --homedir "${GROOVY_GPG_HOME}" --verify "apache-groovy-binary-${VERSION}.zip.asc" "apache-groovy-binary-${VERSION}.zip"
echo "✅ GPG Verified"
...
echo "✅ All binary distribution checks passed successfully for Apache Groovy ${VERSION}."
