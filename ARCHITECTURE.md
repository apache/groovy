...
and
[`subprojects/groovy-lsp/ARCHITECTURE.md`](subprojects/groovy-lsp/ARCHITECTURE.md)
...
- Recognition failures share one message path
  ...
  that has no path to attach to, array-creation mistakes (`new T[]`,
  `new T[][n]`, `new T[n] {…}`), an unclosed type argument, a method
  header in a statement position, an unmatched `do` when `while` is
  expected (a later `while` names a multi-statement body that needs
  `{ }` rather than a missing keyword), enum constants that need `;`
  before members, a misplaced
  `import`, annotation-style `default` after an interface method header,
  named unexpected closers/semicolons, and sole-expected punctuation only
...
- Authored AST nodes carry source positions (`lineNumber` /
  `columnNumber` / `lastLineNumber` / `lastColumnNumber`; 1-based,
  exclusive end column, Unicode code points). Stamp them through
  `PositionConfigureUtils` — do not copy an unpositioned node's end
  onto an authored parent (`DefaultGroovyMethods.asBoolean(Object)` is
  non-null, so a synthetic `ConstantExpression` would otherwise wipe
  `lastLineNumber`). `-1`
  is reserved for synthetic or generated nodes: `ModuleNode`, a
  script's wrapping `BlockStatement`, the implicit `return null`
  singleton, `EmptyExpression.INSTANCE` / `EmptyStatement.INSTANCE`,
  TWR desugaring (`__$$tN`), `@Trait`, and `<clinit>`. Tooling tells
  authored from generated by `lineNumber > 0`
  (`SecureASTCustomizer.isFromSource`). Identifier selection for
  rename lives in `groovy-lsp` (`Positions.toNameRange`), not as
  extra fields on `AnnotatedNode`.
